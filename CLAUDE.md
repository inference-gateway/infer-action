...
Release-candidate dogfood: `.github/workflows/release-candidate.yml` is `workflow_dispatch`-only. It posts a cooking comment, runs the runner with the mock agent against a real issue you supply, then runs post-results. Use it to eyeball comment shape and auto-PR behavior of a build before cutting a release.
