...
`)?f:f+`
`;return Wh(wh(h,o))}import{execFileSync as Rh}from"child_process";import{appendFileSync as $n,existsSync as ao,readFileSync as Yh,rmSync as rh,writeFileSync as Oh}from"fs";var qh=60000;function Hn(){try{Oh(l,"1")}catch(n){console.error("[runner] failed to write cancel marker:",n)}}function Kn(){try{rh(l,{force:!0})}catch{}}function jn(n,h=t){try{return h(`git diff --stat origin/${Dh(n)}...HEAD`)}catch(o){return console.error("[runner] git diff --stat failed:",o),""}}function Jn(n,h=(o)=>o){try{let a=Yh(V,"utf8").split(`
...
`)}function zh(n){let h=n.stderr;return((typeof h==="string"?h:"")+String(n.message)).includes("couldn't find remote ref")}function Th(n,h=t){try{if(n.isFork){let o=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${o}`),h(`git fetch origin pull/${n.prNumber}/head:${o}`),h(`git checkout ${o}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{h(`git fetch origin ${n.headRef}`)}catch(o){if(!zh(o))throw o;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),h(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}h(`git checkout ${n.headRef}`)}}catch(o){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:o})}}async function Uh(n){if(n.exitCode!==null)return n.exitCode;return new Promise((h)=>{n.on("close",(o)=>h(o??0))})}async function Fh(n){if(n.writableFinished)return;await Promise.race([new Promise((h)=>n.once("finish",h)),new Promise((h)=>setTimeout(h,2000).unref())])}function Mh(n){try{zn(B,JSON.stringify(n))}catch(h){console.error("[runner] failed to persist todos:",h)}}function Lh(){try{zn(B,"[]")}catch{}}if(import.meta.main)Xh().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{Qh as resolveMergedSystemPrompt,Gh as renderPlan,Vh as redactMemoryIndex,Th as ensurePrHeadCheckedOut,Zh as buildChildEnv};
