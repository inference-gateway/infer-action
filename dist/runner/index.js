...
This is the user's most recent ask. Address it directly. Do not re-implement existing changes unless this comment asks for that.`:"",i=n.comments.filter((W)=>!W.isTrigger);if(n.reviewComment)return eh(n,n.reviewComment,{forkNotice:o,triggerSection:a,others:i});let s=On(i,"Other comments (chronological)"),u=n.prBody.trim()?n.prBody:"_(no description)_",w=h.trim()?"```\n"+h.trim()+"\n```":"_(no changes on this branch yet)_";return Y("TASK_PR",{prNumber:n.prNumber,prTitle:n.prTitle,headRef:n.headRef,baseRef:n.baseRef,forkNotice:o,prBody:u,triggerSection:a,otherCommentsSection:s,diffStatSection:w})}function eh(n,h,o){let f=h.startLine&&h.line&&h.startLine!==h.line?`, lines ${h.startLine}-${h.line}`:h.line?`, line ${h.line}`:"",a=o.others.length>0?`

## Earlier comments in this review thread

${o.others.map(qn).join(`

`)}`:"";return Y("TASK_PR_REVIEW",{prNumber:n.prNumber,prTitle:n.prTitle,headRef:n.headRef,baseRef:n.baseRef,forkNotice:o.forkNotice,filePath:h.path,lineInfo:f,diffHunk:h.diffHunk,threadSection:a,triggerSection:o.triggerSection})}function qn(n){return`**@${n.author}** \xB7 ${n.createdAt}

${n.body}`}function yh(n,h){let o=[],f=h.enableGitOps&&!(n.kind==="pull_request"&&n.isFork);if(o.push({name:"infer-action-context",hook:"pre_stream",trigger:"interval",interval:5,text:h.enableGitOps?kh(n):"<system-reminder>Keep your TodoWrite plan current as you go. Only answering a question? Ignore this.</system-reminder>"}),f)o.push({name:"infer-action-wrap-up",hook:"pre_stream",trigger:"turns_before_max",threshold:10,text:Wh(n)}),o.push({name:"infer-action-failed-tool",hook:"post_tool",trigger:"on_failure",text:Rh()});return o}function kh(n){if(n.kind==="pull_request"&&n.isFork)return"<system-reminder>This PR is from a fork - you CANNOT commit or push. Investigate with file reads and git diff, then answer the user's question or summarise. Keep your TodoWrite plan current.</system-reminder>";if(n.kind==="pull_request")return`<system-reminder>Keep your TodoWrite plan current. If you changed files, commit + push after each step so PR #${n.prNumber} stays current - unpushed work is lost when the job ends. Only reviewing or answering? Do not change, commit, or push anything.</system-reminder>`;return"<system-reminder>Keep your TodoWrite plan current. Changing code? Work on a pushed branch with an open draft PR (`gh pr create --draft`) and commit + push after each step so nothing is lost - never commit on or push to main. Only answering a question? Ignore this.</system-reminder>"}function Wh(n){return`<system-reminder>You are close to the turn limit. Stop starting new work - if you have uncommitted or unpushed changes, commit and push them now ${n.kind==="pull_request"?`so PR #${n.prNumber} is up to date`:"and make sure the draft PR exists (`gh pr create --draft`)"}. Unpushed work is lost when the run ends. If you changed nothing, just finish your summary.</system-reminder>`}function Rh(){return"<system-reminder>That tool call FAILED - the change did NOT happen. Re-read or re-check, fix it, and retry. Never mark a todo done or claim success on a failed call.</system-reminder>"}function rh(n){let h=["enabled: true","merge: true","reminders:"];for(let o of n){if(h.push(`  - name: ${JSON.stringify(o.name)}`),h.push(`    hook: ${JSON.stringify(o.hook)}`),h.push(`    trigger: ${JSON.stringify(o.trigger)}`),o.interval!==void 0)h.push(`    interval: ${o.interval}`);if(o.threshold!==void 0)h.push(`    threshold: ${o.threshold}`);h.push(`    text: ${JSON.stringify(o.text)}`)}return h.join(`
`)+`
`}function Dn(n,h,o){let f=n.trim();if(f)return f.endsWith(`
`)?f:f+`
`;return rh(yh(h,o))}import{execFileSync as Yh}from"child_process";import{appendFileSync as $n,existsSync as so,readFileSync as Oh,rmSync as qh,writeFileSync as Dh}from"fs";var $h=60000;function Kn(){try{Dh(t,"1")}catch(n){console.error("[runner] failed to write cancel marker:",n)}}function Hn(){try{qh(t,{force:!0})}catch{}}function jn(n,h=l){try{return h(`git diff --stat origin/${Kh(n)}...HEAD`)}catch(o){return console.error("[runner] git diff --stat failed:",o),""}}function Jn(n,h=(o)=>o){try{let a=Oh(V,"utf8").split(`
`).filter((i)=>i.trim()!=="").slice(-n);if(a.length===0)return;console.error("=========================================="),console.error(`[recover] last ${a.length} line(s) of agent activity before it stopped:`),console.error("------------------------------------------");for(let i of a){let s=i.length>2000?i.slice(0,2000)+" \u2026":i;console.error(h(s))}console.error("==========================================")}catch(o){console.error("[recover] could not read agent transcript for breadcrumb:",o)}}function l(n){return Yh("bash",["-c",n],{encoding:"utf8",timeout:$h,env:{...process.env,GIT_TERMINAL_PROMPT:"0"}})}function Kh(n){return`'${n.replace(/'/g,"'\\''")}'`}function H(n,h){let o=process.env.GITHUB_OUTPUT;if(!o){console.log(`(would set output) ${n}=${h}`);return}if(h.includes(`
`)){let f=`_GHO_EOF_${Math.random().toString(36).slice(2)}`;$n(o,`${n}<<${f}
${h}
${f}
`)}else $n(o,`${n}=${h}
`)}function Zn(n){return typeof n==="object"&&n!==null&&n.role==="tool"&&typeof n.content==="string"}function Qn(n){if(typeof n!=="object"||n===null)return!1;let h=n.type;return h==="compaction_started"||h==="compaction_completed"}var Xn="Result of tool call: ";function Vn(n){if(!n.startsWith(Xn))return null;let h=n.slice(Xn.length);try{let o=JSON.parse(h);if(typeof o==="object"&&o!==null)return o;return null}catch{return null}}class A{handlers=new Map;flushers=[];listeners=[];on(n,h){return this.handlers.set(n,h),this}onMessage(n){return this.listeners.push(n),this}addFlusher(n){return this.flushers.push(n),this}async observe(n){for await(let h of n){for(let a of this.listeners)try{a(h)}catch(i){console.error("[ticker] message listener threw:",i)}if(!Zn(h))continue;let o=Vn(h.content);if(!o?.tool_name)continue;let f=this.handlers.get(o.tool_name);if(!f)continue;try{await f(o,h)}catch(a){console.error(`[ticker] handler for ${o.tool_name} threw:`,a)}}}async flush(){for(let n of this.flushers)try{await n()}catch(h){console.error("[ticker] flusher threw:",h)}}}function Pn(n,h){let o=null,f=null,a=null,i=async()=>{if(f=null,!o)return;let s=o.value;o=null,a=n(s).catch((u)=>{console.error("[throttle] fn threw:",u)}).finally(()=>{a=null}),await a};return{call(s){if(o={value:s},!f)f=setTimeout(()=>{i()},h)},async flush(){if(f)clearTimeout(f),f=null;if(o)await i();else if(a)await a}}}function zn(n){let h=Math.floor(n/1000);if(h<60)return`${h}s`;let o=Math.floor(h/60),f=h%60;if(o<60)return`${o}m ${f}s`;let a=Math.floor(o/60),i=o%60;return`${a}h ${i}m ${f}s`}var Zh=1500;async function Qh(){let{dryRun:n,enableGitOps:h,redactor:o,github:f}=yn(),a=y("INFER_REVIEW_MODE")==="true",i=h&&!a,s=y("INFER_COOKING_COMMENT_ID"),u=s?Number.parseInt(s,10):0,w=Number.isFinite(u)&&u>0,W=y("INFER_WORKFLOW_URL"),D=Q("INFER_AGENT_MODEL"),O=y("INFER_CUSTOM_INSTRUCTIONS"),Tn=y("INFER_BASH_ALLOW_APPEND"),_=y("INFER_LOGGING_DEBUG")==="true",P=on(process.env),R=await kn(process.env,f,{stepName:"dry-run",failHard:!n});if(R.kind==="pull_request"&&(h||a))try{Uh(R)}catch(e){if(!n)throw e;console.warn("[dry-run] PR head checkout failed; continuing on the current branch:",e instanceof Error?e.message:e)}let Fn=R.kind==="pull_request"?jn(R.baseRef):"",N=Yn(R,O),v=rn(R,{diffStat:Fn});if(i)for(let e of Rn(R)){let k=e.key.replace(/^SYSTEM_/,"").toLowerCase().replace(/_/g,"-");process.stdout.write(`::warning::INFER_PROMPT_OVERRIDE_${e.key} replaces the bundled system prompt (system-prompt-${k} / src/prompts/system-${k}.md) and is missing the git-safety markers: ${e.missing.join(", ")}. The default guards against lost work (branch-first, commit-per-todo, push, draft PR, finish checklist); your override dropped those instructions, so the agent may leave changes uncommitted or unpushed. Re-add them to your override, or use the custom-instructions input to layer extras on top of the default instead of replacing it.
`)}let Un=y("INFER_REMINDERS_CONFIG"),I=Dn(Un,R,{enableGitOps:i}),S=p(i,Tn),z=y("INFER_BIN")||"infer",Mn=y("INFER_NO_COLOR")==="true",g=Vh(process.env,{systemPrompt:N,bashAllowAppend:S,remindersYaml:I,reviewMode:a});console.log("=========================================="),console.log("SYSTEM PROMPT:"),console.log("==========================================");let C=y("INFER_BIN")?void 0:Ph(z,g);if(console.log(C===void 0?N:zh(C,_)),console.log("=========================================="),console.log(""),console.log("Running agent with task:"),console.log(v),console.log("---"),n)console.log("=========================================="),console.log("DRY RUN - the agent would be invoked with:"),console.log("=========================================="),console.log(`Model:        ${D}`),console.log(`Context kind: ${R.kind}`),console.log(`Git ops:      ${h?"enabled":"disabled"}`),console.log(`Review mode:  ${a?"enabled (Edit/Write/Delete disabled)":"disabled"}`),console.log(`INFER_BIN:    ${z}`),console.log("--- REMINDERS (INFER_REMINDERS_CONFIG) ---"),console.log(I),console.log("--- BASH ALLOW-LIST APPEND (added to the CLI read-only baseline) ---"),console.log(S||"(none - CLI read-only baseline only)"),console.log("==========================================");Bh(),Hn();let Ln=Date.now(),d=["agent","-m",D,v];if(Mn)d.push("--no-colors");let r=Hh(z,d,{stdio:["inherit","pipe","pipe"],env:g});if(!r.stdout||!r.stderr)throw Error("child stdio not piped - this should not happen");let m=!1,b=!1,c=(e)=>{if(b)return;b=!0,m=!0,Kn(),console.error(`[runner] received ${e}; stopping the agent so the salvage step can recover its work`);try{r.kill("SIGKILL")}catch(k){console.error("[runner] failed to stop agent child:",k)}};process.once("SIGTERM",()=>c("SIGTERM")),process.once("SIGINT",()=>c("SIGINT"));let j=Jh(V),x=new Xh;if(r.stdout.pipe(j,{end:!1}),P.stdout)r.stdout.pipe(process.stdout,{end:!1});else console.log("[runner] agent stdout muted (set INFER_MIRROR_AGENT_LOGS=true to mirror); stderr still shown, full transcript written to /tmp/agent-output.txt");r.stdout.pipe(x),r.stdout.on("end",()=>j.end()),r.stderr.on("data",(e)=>{if(j.write(e),P.stderr)process.stderr.write(e)});let $=new A,J=w?Pn(async(e)=>{let k=Th(e,W,D);try{await f.updateZone(u,"plan",k),console.log(`[ticker] updated plan section (${e.length} todos)`)}catch(En){console.error("[ticker] PATCH failed:",En)}},Zh):null;if(J)$.addFlusher(J.flush);else console.log("[ticker] no cooking comment; plan mirroring disabled (direct mode)");if($.on("TodoWrite",(e)=>{let k=e.data?.todos;if(!Array.isArray(k))return;if(Eh(k),J)J.call(k)}),_)$.onMessage((e)=>{if(Qn(e)){console.log(e.type==="compaction_started"?"[agent] context compaction started (summarising older turns)\u2026":"[agent] context compaction completed");return}let k=e;if(k.role==="user"&&k.hidden===!0&&k.kind==="system_reminder")console.log("[agent] system reminder injected")});await $.observe(fn(x)),await $.flush();let K=await Mh(r),G=Date.now()-Ln;if(console.log(""),console.log("=========================================="),console.log(`Agent exited with code ${K}`),console.log(`Duration: ${zn(G)}`),console.log("=========================================="),m){if(H("run-duration-ms",String(G)),await Lh(j),P.stdout)Jn(40,o.redact);return console.error("[runner] cancelled mid-run; the salvage step will recover any work and report the timeout"),130}return H("exit-code",String(K)),H("run-duration-ms",String(G)),H("result",K===0?"Agent completed successfully":`Agent failed with exit code ${K}`),K}function Vh(n,h){return{...n,INFER_PROMPTS_AGENT_SYSTEM_PROMPT:h.systemPrompt,INFER_AGENT_SYSTEM_PROMPT_WITH_DEFAULTS:"true",INFER_TOOLS_BASH_ALLOW_APPEND:h.bashAllowAppend,INFER_REMINDERS_CONFIG:h.remindersYaml,...h.reviewMode?{INFER_TOOLS_EDIT_ENABLED:"false",INFER_TOOLS_WRITE_ENABLED:"false",INFER_TOOLS_DELETE_ENABLED:"false"}:{},OTEL_EXPORTER_OTLP_ENDPOINT:n.OTEL_EXPORTER_OTLP_ENDPOINT??"",OTEL_EXPORTER_OTLP_HEADERS:n.OTEL_EXPORTER_OTLP_HEADERS??"",OTEL_SERVICE_NAME:n.OTEL_SERVICE_NAME??"infer-action",OTEL_RESOURCE_ATTRIBUTES:n.OTEL_RESOURCE_ATTRIBUTES??""}}function Ph(n,h){try{let o=jh(n,["debug","agent","system_prompt"],{env:h,encoding:"utf8",timeout:30000});if(o.status!==0||!o.stdout?.trim())return;return o.stdout}catch{return}}function zh(n,h){if(h)return n;let o=n.indexOf("PERSISTENT MEMORY INDEX");if(o===-1)return n;let f=n.slice(o),a=f.indexOf(`

Current date:`),i=a===-1?"":f.slice(a);return`${n.slice(0,o)}PERSISTENT MEMORY INDEX: [redacted - set debug: true to include memory entries]${i}`}function Gh(n,h){let o=[`**Model:** \`${h}\``];if(n)o.push(`[View Job](${n})`);return`${hn}

${o.join(" \xB7 ")}`}function Th(n,h,o){let f=Gh(h,o);if(n.length===0)return`${f}
...
`)}function Fh(n){let h=n.stderr;return((typeof h==="string"?h:"")+String(n.message)).includes("couldn't find remote ref")}function Uh(n,h=l){try{if(n.isFork){let o=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${o}`),h(`git fetch origin pull/${n.prNumber}/head:${o}`),h(`git checkout ${o}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{h(`git fetch origin ${n.headRef}`)}catch(o){if(!Fh(o))throw o;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),h(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}h(`git checkout ${n.headRef}`)}}catch(o){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:o})}}async function Mh(n){if(n.exitCode!==null)return n.exitCode;return new Promise((h)=>{n.on("close",(o)=>h(o??0))})}async function Lh(n){if(n.writableFinished)return;await Promise.race([new Promise((h)=>n.once("finish",h)),new Promise((h)=>setTimeout(h,2000).unref())])}function Eh(n){try{Gn(B,JSON.stringify(n))}catch(h){console.error("[runner] failed to persist todos:",h)}}function Bh(){try{Gn(B,"[]")}catch{}}if(import.meta.main)Qh().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{Ph as resolveMergedSystemPrompt,Th as renderPlan,zh as redactMemoryIndex,Uh as ensurePrHeadCheckedOut,Vh as buildChildEnv};
