...
Run \`git diff origin/{{baseRef}}...HEAD\` for the full diff and \`git log origin/{{baseRef}}..HEAD\` for the commit history.{{triggerSection}}`};function Ln(n){let o=process.env[`INFER_PROMPT_OVERRIDE_${n}`];return o&&o.trim()?o:en[n]}function An(n){let o=process.env[`INFER_PROMPT_OVERRIDE_${n}`];return o&&o.trim()?o:null}var dn={SYSTEM_ISSUE:["git commit","git push","gh pr create","gh pr ready","git status"],SYSTEM_DIRECT:["git commit","git push","gh pr create","gh pr ready","git status"],SYSTEM_PR:["git commit","git push","git status"],SYSTEM_PR_FORK:["git commit","git push"]};function In(n){if(n.kind==="issue")return"SYSTEM_ISSUE";if(n.kind==="direct")return"SYSTEM_DIRECT";if(n.isFork)return"SYSTEM_PR_FORK";return"SYSTEM_PR"}function an(n){let o=In(n),h=An(o);if(h===null)return[];let e=dn[o];if(!e||e.length===0)return[];let i=e.filter((a)=>!h.includes(a));return i.length>0?[{key:o,missing:i}]:[]}function W(n,o={}){return Ln(n).replace(/\{\{(\w+)\}\}/g,(h,e)=>{if(!(e in o))throw Error(`Missing variable "${e}" for prompt "${n}"`);return String(o[e])})}function sn(n,o={}){if(n.kind==="issue")return Cn(n);if(n.kind==="direct")return Nn(n);return cn(n,o.diffStat??"")}function fn(n,o){let h=En(n);if(o.trim())return`${h}

## Additional Instructions

${o}`;return h}function En(n){if(n.kind==="issue")return W("SYSTEM_ISSUE",{issueNumber:n.issueNumber});if(n.kind==="direct")return W("SYSTEM_DIRECT");if(n.isFork)return W("SYSTEM_PR_FORK",{prNumber:n.prNumber,headRef:n.headRef,headRepoFullName:n.headRepoFullName,baseRef:n.baseRef});return W("SYSTEM_PR",{prNumber:n.prNumber,headRef:n.headRef})}function Nn(n){return W("TASK_DIRECT",{prompt:n.prompt})}function Cn(n){let o=n.triggeringComment?`
...
`)?e:e+`
`;return oo(bn(o,h))}import{execFileSync as ho}from"child_process";import{appendFileSync as un,existsSync as Io,readFileSync as eo,rmSync as ao,writeFileSync as io}from"fs";var so=60000;function tn(){try{io(T,"1")}catch(n){console.error("[runner] failed to write cancel marker:",n)}}function wn(){try{ao(T,{force:!0})}catch{}}function ln(n,o=V){try{return o(`git diff --stat origin/${fo(n)}...HEAD`)}catch(h){return console.error("[runner] git diff --stat failed:",h),""}}function yn(n,o=(h)=>h){try{let i=eo(J,"utf8").split(`
`).filter((a)=>a.trim()!=="").slice(-n);if(i.length===0)return;console.error("=========================================="),console.error(`[recover] last ${i.length} line(s) of agent activity before it stopped:`),console.error("------------------------------------------");for(let a of i){let s=a.length>2000?a.slice(0,2000)+" \u2026":a;console.error(o(s))}console.error("==========================================")}catch(h){console.error("[recover] could not read agent transcript for breadcrumb:",h)}}function V(n){return ho("bash",["-c",n],{encoding:"utf8",timeout:so,env:{...process.env,GIT_TERMINAL_PROMPT:"0"}})}function fo(n){return`'${n.replace(/'/g,"'\\''")}'`}function H(n,o){let h=process.env.GITHUB_OUTPUT;if(!h){console.log(`(would set output) ${n}=${o}`);return}if(o.includes(`
`)){let e=`_GHO_EOF_${Math.random().toString(36).slice(2)}`;un(h,`${n}<<${e}
${o}
${e}
`)}else un(h,`${n}=${o}
`)}function Rn(n){return typeof n==="object"&&n!==null&&n.role==="tool"&&typeof n.content==="string"}function Wn(n){if(typeof n!=="object"||n===null)return!1;let o=n.type;return o==="compaction_started"||o==="compaction_completed"}var kn="Result of tool call: ";function Yn(n){if(!n.startsWith(kn))return null;let o=n.slice(kn.length);try{let h=JSON.parse(o);if(typeof h==="object"&&h!==null)return h;return null}catch{return null}}class z{handlers=new Map;flushers=[];listeners=[];on(n,o){return this.handlers.set(n,o),this}onMessage(n){return this.listeners.push(n),this}addFlusher(n){return this.flushers.push(n),this}async observe(n){for await(let o of n){for(let i of this.listeners)try{i(o)}catch(a){console.error("[ticker] message listener threw:",a)}if(!Rn(o))continue;let h=Yn(o.content);if(!h?.tool_name)continue;let e=this.handlers.get(h.tool_name);if(!e)continue;try{await e(h,o)}catch(i){console.error(`[ticker] handler for ${h.tool_name} threw:`,i)}}}async flush(){for(let n of this.flushers)try{await n()}catch(o){console.error("[ticker] flusher threw:",o)}}}function qn(n,o){let h=null,e=null,i=null,a=async()=>{if(e=null,!h)return;let s=h.value;h=null,i=n(s).catch((r)=>{console.error("[throttle] fn threw:",r)}).finally(()=>{i=null}),await i};return{call(s){if(h={value:s},!e)e=setTimeout(()=>{a()},o)},async flush(){if(e)clearTimeout(e),e=null;if(h)await a();else if(i)await i}}}function Hn(n){let o=Math.floor(n/1000);if(o<60)return`${o}s`;let h=Math.floor(o/60),e=o%60;if(h<60)return`${h}m ${e}s`;let i=Math.floor(h/60),a=h%60;return`${i}h ${a}m ${e}s`}var wo=1500;async function lo(){let{dryRun:n,enableGitOps:o,redactor:h,github:e}=on(),i=l("INFER_COOKING_COMMENT_ID"),a=i?Number.parseInt(i,10):0,s=Number.isFinite(a)&&a>0,r=l("INFER_WORKFLOW_URL"),f=j("INFER_AGENT_MODEL"),w=l("INFER_CUSTOM_INSTRUCTIONS"),R=l("INFER_BASH_ALLOW_APPEND"),Dn=l("INFER_LOGGING_DEBUG")==="true",g=c(process.env),y=await hn(process.env,e,{stepName:"dry-run",failHard:!n});if(y.kind==="pull_request"&&o)Wo(y);let On=y.kind==="pull_request"?ln(y.baseRef):"",v=fn(y,w),M=sn(y,{diffStat:On});if(o)for(let u of an(y)){let t=u.key.replace(/^SYSTEM_/,"").toLowerCase().replace(/_/g,"-");process.stdout.write(`::warning::INFER_PROMPT_OVERRIDE_${u.key} replaces the bundled system prompt (system-prompt-${t} / src/prompts/system-${t}.md) and is missing the git-safety markers: ${u.missing.join(", ")}. The default guards against lost work (branch-first, commit-per-todo, push, draft PR, finish checklist); your override dropped those instructions, so the agent may leave changes uncommitted or unpushed. Re-add them to your override, or use the custom-instructions input to layer extras on top of the default instead of replacing it.
`)}let Kn=l("INFER_REMINDERS_CONFIG"),m=rn(Kn,y,{enableGitOps:o}),B=N(o,R),L=l("INFER_BIN")||"infer";if(console.log("=========================================="),console.log("SYSTEM PROMPT:"),console.log("=========================================="),console.log(v),console.log("=========================================="),console.log(""),console.log("Running agent with task:"),console.log(M),console.log("---"),n)console.log("=========================================="),console.log("DRY RUN - the agent would be invoked with:"),console.log("=========================================="),console.log(`Model:        ${f}`),console.log(`Context kind: ${y.kind}`),console.log(`Git ops:      ${o?"enabled":"disabled"}`),console.log(`INFER_BIN:    ${L}`),console.log("--- REMINDERS (INFER_REMINDERS_CONFIG) ---"),console.log(m),console.log("--- BASH ALLOW-LIST APPEND (added to the CLI read-only baseline) ---"),console.log(B||"(none - CLI read-only baseline only)"),console.log("==========================================");let jn={...process.env,INFER_AGENT_SYSTEM_PROMPT:v,INFER_AGENT_SYSTEM_PROMPT_WITH_DEFAULTS:"true",INFER_TOOLS_BASH_ALLOW_APPEND:B,INFER_REMINDERS_CONFIG:m};$o(),wn();let Jn=Date.now(),k=ro(L,["agent","-m",f,M],{stdio:["inherit","pipe","pipe"],env:jn});if(!k.stdout||!k.stderr)throw Error("child stdio not piped - this should not happen");let A=!1,d=!1,I=(u)=>{if(d)return;d=!0,A=!0,tn(),console.error(`[runner] received ${u}; stopping the agent so the salvage step can recover its work`);try{k.kill("SIGKILL")}catch(t){console.error("[runner] failed to stop agent child:",t)}};process.once("SIGTERM",()=>I("SIGTERM")),process.once("SIGINT",()=>I("SIGINT"));let $=uo(J),E=new to;if(k.stdout.pipe($,{end:!1}),g.stdout)k.stdout.pipe(process.stdout,{end:!1});else console.log("[runner] agent stdout muted (set INFER_MIRROR_AGENT_LOGS=true to mirror); stderr still shown, full transcript written to /tmp/agent-output.txt");k.stdout.pipe(E),k.stdout.on("end",()=>$.end()),k.stderr.on("data",(u)=>{if($.write(u),g.stderr)process.stderr.write(u)});let Y=new z,D=s?qn(async(u)=>{let t=ko(u,r,f);try{await e.updateZone(a,"plan",t),console.log(`[ticker] updated plan section (${u.length} todos)`)}catch(Xn){console.error("[ticker] PATCH failed:",Xn)}},wo):null;if(D)Y.addFlusher(D.flush);else console.log("[ticker] no cooking comment; plan mirroring disabled (direct mode)");if(Y.on("TodoWrite",(u)=>{let t=u.data?.todos;if(!Array.isArray(t))return;if(Ho(t),D)D.call(t)}),Dn)Y.onMessage((u)=>{if(Wn(u)){console.log(u.type==="compaction_started"?"[agent] context compaction started (summarising older turns)\u2026":"[agent] context compaction completed");return}let t=u;if(t.role==="user"&&t.hidden===!0&&t.kind==="system_reminder")console.log("[agent] system reminder injected")});await Y.observe(S(E)),await Y.flush();let q=await Yo(k),X=Date.now()-Jn;if(console.log(""),console.log("=========================================="),console.log(`Agent exited with code ${q}`),console.log(`Duration: ${Hn(X)}`),console.log("=========================================="),A)return H("run-duration-ms",String(X)),await qo($),yn(40,h.redact),console.error("[runner] cancelled mid-run; the salvage step will recover any work and report the timeout"),130;return H("exit-code",String(q)),H("run-duration-ms",String(X)),H("result",q===0?"Agent completed successfully":`Agent failed with exit code ${q}`),q}function yo(n,o){let h=[`**Model:** \`${o}\``];if(n)h.push(`[View Job](${n})`);return`${_}

${h.join(" \xB7 ")}`}function ko(n,o,h){let e=yo(o,h);if(n.length===0)return`${e}
...
`)}function Ro(n){let o=n.stderr;return((typeof o==="string"?o:"")+String(n.message)).includes("couldn't find remote ref")}function Wo(n,o=V){try{if(n.isFork){let h=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${h}`),o(`git fetch origin pull/${n.prNumber}/head:${h}`),o(`git checkout ${h}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{o(`git fetch origin ${n.headRef}`)}catch(h){if(!Ro(h))throw h;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),o(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}o(`git checkout ${n.headRef}`)}}catch(h){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:h})}}async function Yo(n){if(n.exitCode!==null)return n.exitCode;return new Promise((o)=>{n.on("close",(h)=>o(h??0))})}async function qo(n){if(n.writableFinished)return;await Promise.race([new Promise((o)=>n.once("finish",o)),new Promise((o)=>setTimeout(o,2000).unref())])}function Ho(n){try{$n(P,JSON.stringify(n))}catch(o){console.error("[runner] failed to persist todos:",o)}}function $o(){try{$n(P,"[]")}catch{}}if(import.meta.main)lo().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{ko as renderPlan,Wo as ensurePrHeadCheckedOut};
