...
${o}`),{number:0,url:"(dry-run)",body:o,baseRef:n.base};let h=await this.api.pulls.create({owner:this.owner,repo:this.repoName,head:n.head,base:n.base,title:n.title,body:o,draft:!0});return{number:h.data.number,url:h.data.html_url,body:h.data.body??"",baseRef:h.data.base.ref}}async getDefaultBranch(){return(await this.api.repos.get({owner:this.owner,repo:this.repoName})).data.default_branch}async getPullRequest(n){let o=await this.api.pulls.get({owner:this.owner,repo:this.repoName,pull_number:n});return{title:o.data.title,body:o.data.body??"",headRef:o.data.head.ref,headRepoFullName:o.data.head.repo?.full_name??"",baseRef:o.data.base.ref}}async listIssueComments(n){let o=[],h=2;for(let e=1;e<=2;e++){let i=await this.api.issues.listComments({owner:this.owner,repo:this.repoName,issue_number:n,per_page:100,page:e});for(let a of i.data)o.push({id:a.id,author:a.user?.login??"unknown",body:a.body??"",createdAt:a.created_at});if(i.data.length<100)break}return o}}function p(n){return{stdout:n.INFER_MIRROR_AGENT_LOGS==="true",stderr:!0}}import Tn from"readline";async function*x(n){let o=Tn.createInterface({input:n,crlfDelay:1/0});for await(let h of o){let e=h.trim();if(!e)continue;if(e[0]!=="{")continue;try{let i=JSON.parse(e);if(typeof i!=="object"||i===null)continue;let{role:a,type:s}=i;if(typeof a==="string"||s==="session_stats"||s==="compaction_started"||s==="compaction_completed")yield i}catch{}}}async function nn(n,o){let h=n.INFER_CONTEXT_KIND;if(!h)throw Error("Missing required env var INFER_CONTEXT_KIND");if(h==="issue")return zn(n,o);if(h==="pull_request")return gn(n,o);if(h==="direct")return Vn(n);throw Error(`Unknown INFER_CONTEXT_KIND "${h}" (expected "issue", "pull_request", or "direct")`)}function on(n){let o=n.INFER_CONTEXT_KIND;if(o==="direct")return{kind:"direct",prompt:(n.INFER_DIRECT_PROMPT??"").trim()||"(dry-run: no prompt)"};if(o==="pull_request")return{kind:"pull_request",prNumber:Number.parseInt(n.INFER_ISSUE_NUMBER??"0",10)||0,prTitle:"(dry-run: PR title unavailable)",prBody:"",headRef:"(unknown)",baseRef:"main",headRepoFullName:"",isFork:!1,triggeringCommentId:0,comments:[]};return{kind:"issue",issueNumber:Number.parseInt(n.INFER_ISSUE_NUMBER??"0",10)||0,issueTitle:n.INFER_ISSUE_TITLE??"",issueBody:n.INFER_ISSUE_BODY??""}}function Vn(n){let o=(n.INFER_DIRECT_PROMPT??"").trim();if(!o)throw Error("Missing or empty INFER_DIRECT_PROMPT for direct context");return{kind:"direct",prompt:o}}async function zn(n,o){let h=Number.parseInt(n.INFER_ISSUE_NUMBER??"",10);if(!Number.isFinite(h))throw Error("Missing or invalid INFER_ISSUE_NUMBER");let e=n.INFER_ISSUE_TITLE??"",i=n.INFER_ISSUE_BODY??"",a=Mn(n),{associatedPrs:s,associatedBranches:u}=await Pn(o,h);return{kind:"issue",issueNumber:h,issueTitle:e,issueBody:i,...a?{triggeringComment:a}:{},...s.length?{associatedPrs:s}:{},...u.length?{associatedBranches:u}:{}}}async function Pn(n,o){let h=`fix/issue-${o}`;try{let[e,i]=await Promise.all([n.getOpenPrForBranch(h),n.findPrsReferencingIssue(o)]),a=new Map;for(let f of i)a.set(f.number,f);if(e){let f=a.get(e.number);a.set(e.number,{number:e.number,url:f?.url||e.url,state:f?.state||"open",headRef:h,baseRef:e.baseRef,isDraft:f?.isDraft??!1,title:f?.title??""})}return{associatedPrs:[...a.values()],associatedBranches:e?[h]:[]}}catch(e){return console.warn(`[context] failed to gather existing work for issue #${o}; proceeding without it:`,e instanceof Error?e.message:e),{associatedPrs:[],associatedBranches:[]}}}async function gn(n,o){let h=Number.parseInt(n.INFER_ISSUE_NUMBER??"",10);if(!Number.isFinite(h))throw Error("Missing or invalid INFER_ISSUE_NUMBER for PR context");let[e,i]=await Promise.all([o.getPullRequest(h),o.listIssueComments(h)]),a=Number.parseInt(n.INFER_TRIGGERING_COMMENT_ID??"",10),s=Number.isFinite(a)?a:0,u=i.map((R)=>({id:R.id,author:R.author,body:R.body,createdAt:R.createdAt,isTrigger:s>0&&R.id===s})),f=`${o.owner}/${o.repoName}`,w=e.headRepoFullName!==""&&e.headRepoFullName!==f;return{kind:"pull_request",prNumber:h,prTitle:e.title,prBody:e.body,headRef:e.headRef,baseRef:e.baseRef,headRepoFullName:e.headRepoFullName,isFork:w,triggeringCommentId:s,comments:u}}function Mn(n){let o=n.INFER_TRIGGERING_COMMENT_ID??"",h=n.INFER_TRIGGERING_COMMENT_BODY??"",e=n.INFER_TRIGGERING_COMMENT_AUTHOR??"",i=Number.parseInt(o,10);if(!Number.isFinite(i)||i<=0)return;if(!h.trim())return;return{id:i,body:h,author:e}}var F=["GITHUB_TOKEN","OLLAMA_API_KEY","OLLAMA_CLOUD_API_KEY","GROQ_API_KEY","OPENAI_API_KEY","CLOUDFLARE_API_KEY","COHERE_API_KEY","ANTHROPIC_API_KEY","DEEPSEEK_API_KEY","GOOGLE_API_KEY","MISTRAL_API_KEY","MINIMAX_API_KEY","MOONSHOT_API_KEY","NVIDIA_API_KEY","CLAUDE_CODE_OAUTH_TOKEN","OTEL_EXPORTER_OTLP_HEADERS","MEMORY_TOKEN","MEMORY_DEPLOY_KEY"],vn=["-----BEGIN [A-Z ]*PRIVATE KEY( BLOCK)?-----[\\s\\S]+?-----END [A-Z ]*PRIVATE KEY( BLOCK)?-----"],Ln=["github_pat_[A-Za-z0-9_]{82,}","gh[pours]_[A-Za-z0-9]{36,}","AIza[0-9A-Za-z_-]{35}","xox[bpoa]-[A-Za-z0-9-]{20,}","sk-[A-Za-z0-9_-]{20,}","eyJ[A-Za-z0-9_-]+\\.eyJ[A-Za-z0-9_-]+\\.[A-Za-z0-9_-]{10,}"];var An=/[.*+?^${}()|[\]\\]/g;function T(n,o,h=8){let e=[],i=new Set;for(let a of o){let s=n[a];if(typeof s!=="string")continue;if(s.trim().length<h)continue;if(i.has(s))continue;i.add(s),e.push(s)}return e}function hn(n){let o=new Set;for(let h of n){if(!h||o.has(h))continue;o.add(h),process.stdout.write(`::add-mask::${h}
`)}}function en(n={}){let o=n.placeholder??"***",h=n.minLength??8,e=n.env??process.env,i=n.heuristics??!1,a=T(e,F,h);a.sort((f,w)=>w.length-f.length);let s=a.map(Bn);if(s.push(...vn),i)s.push(...Ln);let u=s.length>0?new RegExp(s.join("|"),"g"):null;return{secretCount:a.length,redact(f){if(!u||!f)return f;return f.replace(u,o)}}}function Bn(n){return n.replace(An,"\\$&")}var J="/tmp/agent-output.txt",V="/tmp/infer-todos.json",z="/tmp/infer-cancelled";function j(n){let o=process.env[n];if(!o)throw Error(`Missing required env var ${n}`);return o}function y(n){return process.env[n]??""}function an(){let n=y("INFER_DRY_RUN")==="true",o=n?y("GITHUB_TOKEN"):j("GITHUB_TOKEN"),h=j("INFER_REPO"),e=y("INFER_ENABLE_GIT_OPERATIONS")!=="false",i=y("INFER_REDACT_HEURISTICS")==="true",a=T(process.env,F);hn(a);let s=en({env:process.env,heuristics:i}),u=new U({token:o,repo:h,redactor:s,dryRun:n});return{dryRun:n,token:o,repo:h,enableGitOps:e,enableHeuristics:i,redactor:s,github:u}}async function sn(n,o,h){try{return await nn(n,o)}catch(e){if(h.failHard)throw e;return console.warn(`[${h.stepName}] context read failed (${e.message}); proceeding with env-derived data`),on(n)}}var fn={SYSTEM_DIRECT:`# Infer Agent (manual run)
...
Run \`git diff origin/{{baseRef}}...HEAD\` for the full diff and \`git log origin/{{baseRef}}..HEAD\` for the commit history.{{triggerSection}}`};function In(n){let o=process.env[`INFER_PROMPT_OVERRIDE_${n}`];return o&&o.trim()?o:fn[n]}function En(n){let o=process.env[`INFER_PROMPT_OVERRIDE_${n}`];return o&&o.trim()?o:null}var Nn={SYSTEM_ISSUE:["git commit","git push","gh pr create","gh pr ready","git status"],SYSTEM_DIRECT:["git commit","git push","gh pr create","gh pr ready","git status"],SYSTEM_PR:["git commit","git push","git status"],SYSTEM_PR_FORK:["git commit","git push"]};function dn(n){if(n.kind==="issue")return"SYSTEM_ISSUE";if(n.kind==="direct")return"SYSTEM_DIRECT";if(n.isFork)return"SYSTEM_PR_FORK";return"SYSTEM_PR"}function un(n){let o=dn(n),h=En(o);if(h===null)return[];let e=Nn[o];if(!e||e.length===0)return[];let i=e.filter((a)=>!h.includes(a));return i.length>0?[{key:o,missing:i}]:[]}function W(n,o={}){return In(n).replace(/\{\{(\w+)\}\}/g,(h,e)=>{if(!(e in o))throw Error(`Missing variable "${e}" for prompt "${n}"`);return String(o[e])})}function rn(n,o={}){if(n.kind==="issue")return _n(n);if(n.kind==="direct")return Cn(n);return Sn(n,o.diffStat??"")}function tn(n,o){let h=mn(n);if(o.trim())return`${h}

## Additional Instructions

${o}`;return h}function mn(n){if(n.kind==="issue")return W("SYSTEM_ISSUE",{issueNumber:n.issueNumber});if(n.kind==="direct")return W("SYSTEM_DIRECT");if(n.isFork)return W("SYSTEM_PR_FORK",{prNumber:n.prNumber,headRef:n.headRef,headRepoFullName:n.headRepoFullName,baseRef:n.baseRef});return W("SYSTEM_PR",{prNumber:n.prNumber,headRef:n.headRef})}function Cn(n){return W("TASK_DIRECT",{prompt:n.prompt})}function _n(n){let o=n.triggeringComment?`

## Triggering comment from @${n.triggeringComment.author}

${n.triggeringComment.body}

Treat this comment as the user's most recent intent. If it asks for something more specific than the issue body, prioritise it.`:"";return W("TASK_ISSUE",{issueNumber:n.issueNumber,issueTitle:n.issueTitle,issueBody:n.issueBody,existingWorkSection:cn(n),triggeringCommentSection:o})}function cn(n){let o=n.associatedPrs??[],h=n.associatedBranches??[];if(o.length===0&&h.length===0)return"";let e=["## Existing work for this issue","A prior run or another contributor may already have started on this issue. Before creating a branch, inspect the items below and CONTINUE from them if they contain relevant work - check it out (`gh pr checkout <number>`, or `git fetch origin <branch> && git checkout <branch>`) and build on top of it rather than starting fresh. Only start a new branch if none of these apply."];if(o.length){let i=o.map((a)=>{let s=a.isDraft?" (draft)":"",u=a.state&&a.state!=="open"?` [${a.state}]`:"",f=a.headRef?` - branch \`${a.headRef}\``:"",w=a.title?` - ${a.title}`:"";return`- PR #${a.number}${s}${u}${f}: ${a.url}${w}`});e.push(`### Pull requests
...
`)}function Sn(n,o){let h=n.isFork?`
...
`)}`:"",u=n.prBody.trim()?n.prBody:"_(no description)_",f=o.trim()?"```\n"+o.trim()+"\n```":"_(no changes on this branch yet)_";return W("TASK_PR",{prNumber:n.prNumber,prTitle:n.prTitle,headRef:n.headRef,baseRef:n.baseRef,forkNotice:h,prBody:u,triggerSection:i,otherCommentsSection:s,diffStatSection:f})}function bn(n){return`**@${n.author}** \xB7 ${n.createdAt}

${n.body}`}function pn(n,o){let h=[],e=o.enableGitOps&&!(n.kind==="pull_request"&&n.isFork);if(h.push({name:"infer-action-context",hook:"pre_stream",trigger:"interval",interval:5,text:o.enableGitOps?xn(n):"<system-reminder>Keep your TodoWrite plan current as you go. Only answering a question? Ignore this.</system-reminder>"}),e)h.push({name:"infer-action-wrap-up",hook:"pre_stream",trigger:"turns_before_max",threshold:10,text:no(n)}),h.push({name:"infer-action-failed-tool",hook:"post_tool",trigger:"on_failure",text:oo()});return h}function xn(n){if(n.kind==="pull_request"&&n.isFork)return"<system-reminder>This PR is from a fork - you CANNOT commit or push. Investigate with file reads and git diff, then answer the user's question or summarise. Keep your TodoWrite plan current.</system-reminder>";if(n.kind==="pull_request")return`<system-reminder>Keep your TodoWrite plan current, and commit + push after each step so PR #${n.prNumber} stays current - unpushed work is lost when the job ends.</system-reminder>`;return"<system-reminder>Keep your TodoWrite plan current. Changing code? Work on a pushed branch with an open draft PR (`gh pr create --draft`) and commit + push after each step so nothing is lost - never commit on or push to main. Only answering a question? Ignore this.</system-reminder>"}function no(n){return`<system-reminder>You are close to the turn limit. Stop starting new work - commit and push everything now ${n.kind==="pull_request"?`so PR #${n.prNumber} is up to date`:"and make sure the draft PR exists (`gh pr create --draft`)"}. Unpushed work is lost when the run ends.</system-reminder>`}function oo(){return"<system-reminder>That tool call FAILED - the change did NOT happen. Re-read or re-check, fix it, and retry. Never mark a todo done or claim success on a failed call.</system-reminder>"}function ho(n){let o=["enabled: true","merge: true","reminders:"];for(let h of n){if(o.push(`  - name: ${JSON.stringify(h.name)}`),o.push(`    hook: ${JSON.stringify(h.hook)}`),o.push(`    trigger: ${JSON.stringify(h.trigger)}`),h.interval!==void 0)o.push(`    interval: ${h.interval}`);if(h.threshold!==void 0)o.push(`    threshold: ${h.threshold}`);o.push(`    text: ${JSON.stringify(h.text)}`)}return o.join(`
`)+`
`}function wn(n,o,h){let e=n.trim();if(e)return e.endsWith(`
`)?e:e+`
`;return ho(pn(o,h))}import{execFileSync as eo}from"child_process";import{appendFileSync as yn,existsSync as co,readFileSync as ao,rmSync as io,writeFileSync as so}from"fs";var fo=60000;function ln(){try{so(z,"1")}catch(n){console.error("[runner] failed to write cancel marker:",n)}}function kn(){try{io(z,{force:!0})}catch{}}function Rn(n,o=P){try{return o(`git diff --stat origin/${uo(n)}...HEAD`)}catch(h){return console.error("[runner] git diff --stat failed:",h),""}}function Wn(n,o=(h)=>h){try{let i=ao(J,"utf8").split(`
`).filter((a)=>a.trim()!=="").slice(-n);if(i.length===0)return;console.error("=========================================="),console.error(`[recover] last ${i.length} line(s) of agent activity before it stopped:`),console.error("------------------------------------------");for(let a of i){let s=a.length>2000?a.slice(0,2000)+" \u2026":a;console.error(o(s))}console.error("==========================================")}catch(h){console.error("[recover] could not read agent transcript for breadcrumb:",h)}}function P(n){return eo("bash",["-c",n],{encoding:"utf8",timeout:fo,env:{...process.env,GIT_TERMINAL_PROMPT:"0"}})}function uo(n){return`'${n.replace(/'/g,"'\\''")}'`}function H(n,o){let h=process.env.GITHUB_OUTPUT;if(!h){console.log(`(would set output) ${n}=${o}`);return}if(o.includes(`
`)){let e=`_GHO_EOF_${Math.random().toString(36).slice(2)}`;yn(h,`${n}<<${e}
${o}
${e}
`)}else yn(h,`${n}=${o}
`)}function qn(n){return typeof n==="object"&&n!==null&&n.role==="tool"&&typeof n.content==="string"}function Hn(n){if(typeof n!=="object"||n===null)return!1;let o=n.type;return o==="compaction_started"||o==="compaction_completed"}var Yn="Result of tool call: ";function $n(n){if(!n.startsWith(Yn))return null;let o=n.slice(Yn.length);try{let h=JSON.parse(o);if(typeof h==="object"&&h!==null)return h;return null}catch{return null}}class g{handlers=new Map;flushers=[];listeners=[];on(n,o){return this.handlers.set(n,o),this}onMessage(n){return this.listeners.push(n),this}addFlusher(n){return this.flushers.push(n),this}async observe(n){for await(let o of n){for(let i of this.listeners)try{i(o)}catch(a){console.error("[ticker] message listener threw:",a)}if(!qn(o))continue;let h=$n(o.content);if(!h?.tool_name)continue;let e=this.handlers.get(h.tool_name);if(!e)continue;try{await e(h,o)}catch(i){console.error(`[ticker] handler for ${h.tool_name} threw:`,i)}}}async flush(){for(let n of this.flushers)try{await n()}catch(o){console.error("[ticker] flusher threw:",o)}}}function Dn(n,o){let h=null,e=null,i=null,a=async()=>{if(e=null,!h)return;let s=h.value;h=null,i=n(s).catch((u)=>{console.error("[throttle] fn threw:",u)}).finally(()=>{i=null}),await i};return{call(s){if(h={value:s},!e)e=setTimeout(()=>{a()},o)},async flush(){if(e)clearTimeout(e),e=null;if(h)await a();else if(i)await i}}}function On(n){let o=Math.floor(n/1000);if(o<60)return`${o}s`;let h=Math.floor(o/60),e=o%60;if(h<60)return`${h}m ${e}s`;let i=Math.floor(h/60),a=h%60;return`${i}h ${a}m ${e}s`}var lo=1500;async function ko(){let{dryRun:n,enableGitOps:o,redactor:h,github:e}=an(),i=y("INFER_COOKING_COMMENT_ID"),a=i?Number.parseInt(i,10):0,s=Number.isFinite(a)&&a>0,u=y("INFER_WORKFLOW_URL"),f=j("INFER_AGENT_MODEL"),w=y("INFER_CUSTOM_INSTRUCTIONS"),R=y("INFER_BASH_ALLOW_APPEND"),M=y("INFER_LOGGING_DEBUG")==="true",v=p(process.env),l=await sn(process.env,e,{stepName:"dry-run",failHard:!n});if(l.kind==="pull_request"&&o)Do(l);let jn=l.kind==="pull_request"?Rn(l.baseRef):"",L=tn(l,w),A=rn(l,{diffStat:jn});if(o)for(let r of un(l)){let t=r.key.replace(/^SYSTEM_/,"").toLowerCase().replace(/_/g,"-");process.stdout.write(`::warning::INFER_PROMPT_OVERRIDE_${r.key} replaces the bundled system prompt (system-prompt-${t} / src/prompts/system-${t}.md) and is missing the git-safety markers: ${r.missing.join(", ")}. The default guards against lost work (branch-first, commit-per-todo, push, draft PR, finish checklist); your override dropped those instructions, so the agent may leave changes uncommitted or unpushed. Re-add them to your override, or use the custom-instructions input to layer extras on top of the default instead of replacing it.
`)}let Jn=y("INFER_REMINDERS_CONFIG"),B=wn(Jn,l,{enableGitOps:o}),I=c(o,R),X=y("INFER_BIN")||"infer",E=Ro(process.env,{systemPrompt:L,bashAllowAppend:I,remindersYaml:B});console.log("=========================================="),console.log("SYSTEM PROMPT:"),console.log("==========================================");let N=y("INFER_BIN")?void 0:Wo(X,E);if(console.log(N===void 0?L:Yo(N,M)),console.log("=========================================="),console.log(""),console.log("Running agent with task:"),console.log(A),console.log("---"),n)console.log("=========================================="),console.log("DRY RUN - the agent would be invoked with:"),console.log("=========================================="),console.log(`Model:        ${f}`),console.log(`Context kind: ${l.kind}`),console.log(`Git ops:      ${o?"enabled":"disabled"}`),console.log(`INFER_BIN:    ${X}`),console.log("--- REMINDERS (INFER_REMINDERS_CONFIG) ---"),console.log(B),console.log("--- BASH ALLOW-LIST APPEND (added to the CLI read-only baseline) ---"),console.log(I||"(none - CLI read-only baseline only)"),console.log("==========================================");Jo(),kn();let Xn=Date.now(),k=ro(X,["agent","-m",f,A],{stdio:["inherit","pipe","pipe"],env:E});if(!k.stdout||!k.stderr)throw Error("child stdio not piped - this should not happen");let d=!1,m=!1,C=(r)=>{if(m)return;m=!0,d=!0,ln(),console.error(`[runner] received ${r}; stopping the agent so the salvage step can recover its work`);try{k.kill("SIGKILL")}catch(t){console.error("[runner] failed to stop agent child:",t)}};process.once("SIGTERM",()=>C("SIGTERM")),process.once("SIGINT",()=>C("SIGINT"));let $=wo(J),_=new yo;if(k.stdout.pipe($,{end:!1}),v.stdout)k.stdout.pipe(process.stdout,{end:!1});else console.log("[runner] agent stdout muted (set INFER_MIRROR_AGENT_LOGS=true to mirror); stderr still shown, full transcript written to /tmp/agent-output.txt");k.stdout.pipe(_),k.stdout.on("end",()=>$.end()),k.stderr.on("data",(r)=>{if($.write(r),v.stderr)process.stderr.write(r)});let Y=new g,D=s?Dn(async(r)=>{let t=Ho(r,u,f);try{await e.updateZone(a,"plan",t),console.log(`[ticker] updated plan section (${r.length} todos)`)}catch(Zn){console.error("[ticker] PATCH failed:",Zn)}},lo):null;if(D)Y.addFlusher(D.flush);else console.log("[ticker] no cooking comment; plan mirroring disabled (direct mode)");if(Y.on("TodoWrite",(r)=>{let t=r.data?.todos;if(!Array.isArray(t))return;if(jo(t),D)D.call(t)}),M)Y.onMessage((r)=>{if(Hn(r)){console.log(r.type==="compaction_started"?"[agent] context compaction started (summarising older turns)\u2026":"[agent] context compaction completed");return}let t=r;if(t.role==="user"&&t.hidden===!0&&t.kind==="system_reminder")console.log("[agent] system reminder injected")});await Y.observe(x(_)),await Y.flush();let q=await Oo(k),Z=Date.now()-Xn;if(console.log(""),console.log("=========================================="),console.log(`Agent exited with code ${q}`),console.log(`Duration: ${On(Z)}`),console.log("=========================================="),d)return H("run-duration-ms",String(Z)),await Ko($),Wn(40,h.redact),console.error("[runner] cancelled mid-run; the salvage step will recover any work and report the timeout"),130;return H("exit-code",String(q)),H("run-duration-ms",String(Z)),H("result",q===0?"Agent completed successfully":`Agent failed with exit code ${q}`),q}function Ro(n,o){return{...n,INFER_PROMPTS_AGENT_SYSTEM_PROMPT:o.systemPrompt,INFER_PROMPTS_AGENT_SYSTEM_PROMPT_CLAUDE_CODE:o.systemPrompt,INFER_AGENT_SYSTEM_PROMPT_WITH_DEFAULTS:"true",INFER_TOOLS_BASH_ALLOW_APPEND:o.bashAllowAppend,INFER_REMINDERS_CONFIG:o.remindersYaml}}function Wo(n,o){try{let h=to(n,["debug","agent","system_prompt"],{env:o,encoding:"utf8",timeout:30000});if(h.status!==0||!h.stdout?.trim())return;return h.stdout}catch{return}}function Yo(n,o){if(o)return n;let h=n.indexOf("PERSISTENT MEMORY INDEX");if(h===-1)return n;let e=n.slice(h),i=e.indexOf(`

Current date:`),a=i===-1?"":e.slice(i);return`${n.slice(0,h)}PERSISTENT MEMORY INDEX: [redacted - set debug: true to include memory entries]${a}`}function qo(n,o){let h=[`**Model:** \`${o}\``];if(n)h.push(`[View Job](${n})`);return`${b}

${h.join(" \xB7 ")}`}function Ho(n,o,h){let e=qo(o,h);if(n.length===0)return`${e}
...
`)}function $o(n){let o=n.stderr;return((typeof o==="string"?o:"")+String(n.message)).includes("couldn't find remote ref")}function Do(n,o=P){try{if(n.isFork){let h=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${h}`),o(`git fetch origin pull/${n.prNumber}/head:${h}`),o(`git checkout ${h}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{o(`git fetch origin ${n.headRef}`)}catch(h){if(!$o(h))throw h;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),o(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}o(`git checkout ${n.headRef}`)}}catch(h){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:h})}}async function Oo(n){if(n.exitCode!==null)return n.exitCode;return new Promise((o)=>{n.on("close",(h)=>o(h??0))})}async function Ko(n){if(n.writableFinished)return;await Promise.race([new Promise((o)=>n.once("finish",o)),new Promise((o)=>setTimeout(o,2000).unref())])}function jo(n){try{Kn(V,JSON.stringify(n))}catch(o){console.error("[runner] failed to persist todos:",o)}}function Jo(){try{Kn(V,"[]")}catch{}}if(import.meta.main)ko().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{Wo as resolveMergedSystemPrompt,Ho as renderPlan,Yo as redactMemoryIndex,Do as ensurePrHeadCheckedOut,Ro as buildChildEnv};
