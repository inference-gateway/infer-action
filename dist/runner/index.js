...
${i}`);return}let f=await this.getCommentBody(n),a=An(f);a[h]=o,await this.updateCommentBody(n,_n(a))}async clearSpinner(n){if(this.dryRun){console.log(`[dry-run] would clear the spinner on comment #${n} (${this.commentUrl(n)})`);return}let h=await this.getCommentBody(n),o=ln(h);if(o===h)return;await this.updateCommentBody(n,o)}async getOpenPrForBranch(n){let o=(await this.api.pulls.list({owner:this.owner,repo:this.repoName,head:`${this.owner}:${n}`,state:"open",per_page:1})).data[0];if(!o)return null;return{number:o.number,url:o.html_url,body:o.body??"",baseRef:o.base.ref}}async getPrForBranch(n){let h=await this.api.pulls.list({owner:this.owner,repo:this.repoName,head:`${this.owner}:${n}`,state:"all",per_page:20}),o=(s)=>({number:s.number,url:s.html_url,body:s.body??"",baseRef:s.base.ref,state:s.state==="open"?"open":"closed",merged:s.merged_at!=null}),f=h.data.find((s)=>s.state==="open");if(f)return o(f);let a=h.data.find((s)=>s.merged_at!=null);if(a)return o(a);let i=h.data[0];return i?o(i):null}async findPrsReferencingIssue(n){let o=(await this.api.issues.listEventsForTimeline({owner:this.owner,repo:this.repoName,issue_number:n,per_page:100})).data,f=new Map;for(let a of o){if(a.event!=="cross-referenced")continue;let i=a.source?.issue;if(!i||!i.pull_request||typeof i.number!=="number")continue;f.set(i.number,{number:i.number,url:i.html_url??"",state:i.state??"",headRef:"",baseRef:"",isDraft:i.draft??!1,title:i.title??""})}return[...f.values()]}async updatePullRequestBody(n,h){let o=this.redactor?this.redactor.redact(h):h;if(this.dryRun){console.log(`[dry-run] would update PR #${n} body (${this.prUrl(n)}):
...
${h}`),{number:0,url:"(dry-run)",body:h,baseRef:n.base};let o=await this.api.pulls.create({owner:this.owner,repo:this.repoName,head:n.head,base:n.base,title:n.title,body:h,draft:!0});return{number:o.data.number,url:o.data.html_url,body:o.data.body??"",baseRef:o.data.base.ref}}async uploadArtifactImage(n,h,o){let f=`${n}/${encodeURIComponent(h)}`,a=`https://raw.githubusercontent.com/${this.owner}/${this.repoName}/${O}/${n}/${encodeURIComponent(h)}`;if(this.dryRun)return console.log(`[dry-run] would upload artifact image ${h} to branch ${O} at ${f}`),a;try{return await this.ensureArtifactsBranch(),(await this.api.repos.createOrUpdateFileContents({owner:this.owner,repo:this.repoName,path:f,message:`chore: add run artifact ${h} (run ${n})`,content:Buffer.from(o).toString("base64"),branch:O})).data.content?.download_url||a}catch(i){return console.error(`artifact image upload failed for ${h}:`,i),null}}async ensureArtifactsBranch(){if(this.artifactsBranchEnsured)return;try{await this.api.git.getRef({owner:this.owner,repo:this.repoName,ref:`heads/${O}`}),this.artifactsBranchEnsured=!0;return}catch{}let n=await this.api.git.createCommit({owner:this.owner,repo:this.repoName,message:`chore: initialize ${O} branch for run artifacts`,tree:tn});await this.api.git.createRef({owner:this.owner,repo:this.repoName,ref:`refs/heads/${O}`,sha:n.data.sha}),this.artifactsBranchEnsured=!0}async getDefaultBranch(){return(await this.api.repos.get({owner:this.owner,repo:this.repoName})).data.default_branch}async getPullRequest(n){let h=await this.api.pulls.get({owner:this.owner,repo:this.repoName,pull_number:n});return{title:h.data.title,body:h.data.body??"",headRef:h.data.head.ref,headRepoFullName:h.data.head.repo?.full_name??"",baseRef:h.data.base.ref,state:h.data.merged_at?"merged":h.data.state==="open"?"open":"closed"}}async listIssueComments(n){let h=[],o=2;for(let f=1;f<=2;f++){let a=await this.api.issues.listComments({owner:this.owner,repo:this.repoName,issue_number:n,per_page:100,page:f});for(let i of a.data)h.push({id:i.id,author:i.user?.login??"unknown",body:i.body??"",createdAt:i.created_at});if(a.data.length<100)break}return h}async listReviewComments(n){let h=[],o=2;for(let f=1;f<=2;f++){let a=await this.api.pulls.listComments({owner:this.owner,repo:this.repoName,pull_number:n,per_page:100,page:f});for(let i of a.data)h.push({id:i.id,author:i.user?.login??"unknown",body:i.body??"",createdAt:i.created_at,inReplyToId:i.in_reply_to_id??0});if(a.data.length<100)break}return h}}function on(n){return{stdout:n.INFER_MIRROR_AGENT_LOGS==="true",stderr:!0}}import Nn from"readline";async function*fn(n){let h=Nn.createInterface({input:n,crlfDelay:1/0});for await(let o of h){let f=o.trim();if(!f)continue;if(f[0]!=="{")continue;try{let a=JSON.parse(f);if(typeof a!=="object"||a===null)continue;let{role:i,type:s}=a;if(typeof i==="string"||s==="session_stats"||s==="compaction_started"||s==="compaction_completed")yield a}catch{}}}async function an(n,h){let o=n.INFER_CONTEXT_KIND;if(!o)throw Error("Missing required env var INFER_CONTEXT_KIND");if(o==="issue")return In(n,h);if(o==="pull_request")return Cn(n,h);if(o==="direct")return vn(n);throw Error(`Unknown INFER_CONTEXT_KIND "${o}" (expected "issue", "pull_request", or "direct")`)}function sn(n){let h=n.INFER_CONTEXT_KIND;if(h==="direct")return{kind:"direct",prompt:(n.INFER_DIRECT_PROMPT??"").trim()||"(dry-run: no prompt)"};if(h==="pull_request"){let o=un(n),f=o?M(n):void 0;return{kind:"pull_request",prNumber:Number.parseInt(n.INFER_ISSUE_NUMBER??"0",10)||0,prTitle:"(dry-run: PR title unavailable)",prBody:"",headRef:"(unknown)",baseRef:"main",headRepoFullName:"",isFork:!1,prState:"open",triggeringCommentId:f?.id??0,comments:f?[{id:f.id,author:f.author,body:f.body,createdAt:"",isTrigger:!0}]:[],...o?{reviewComment:o}:{}}}return{kind:"issue",issueNumber:Number.parseInt(n.INFER_ISSUE_NUMBER??"0",10)||0,issueTitle:n.INFER_ISSUE_TITLE??"",issueBody:n.INFER_ISSUE_BODY??""}}function vn(n){let h=(n.INFER_DIRECT_PROMPT??"").trim();if(!h)throw Error("Missing or empty INFER_DIRECT_PROMPT for direct context");return{kind:"direct",prompt:h}}async function In(n,h){let o=Number.parseInt(n.INFER_ISSUE_NUMBER??"",10);if(!Number.isFinite(o))throw Error("Missing or invalid INFER_ISSUE_NUMBER");let f=n.INFER_ISSUE_TITLE??"",a=n.INFER_ISSUE_BODY??"",i=M(n),[{associatedPrs:s,associatedBranches:u},w]=await Promise.all([Sn(h,o),gn(h,o,i?.id??0)]);return{kind:"issue",issueNumber:o,issueTitle:f,issueBody:a,...i?{triggeringComment:i}:{},...s.length?{associatedPrs:s}:{},...u.length?{associatedBranches:u}:{},...w.length?{threadComments:w}:{}}}async function gn(n,h,o){try{return(await n.listIssueComments(h)).map((a)=>({id:a.id,author:a.author,body:a.body,createdAt:a.createdAt,isTrigger:o>0&&a.id===o}))}catch(f){return console.warn(`[context] failed to list comments for issue #${h}; proceeding without the thread:`,f instanceof Error?f.message:f),[]}}async function Sn(n,h){let o=`fix/issue-${h}`;try{let[f,a]=await Promise.all([n.getOpenPrForBranch(o),n.findPrsReferencingIssue(h)]),i=new Map;for(let w of a)i.set(w.number,w);if(f){let w=i.get(f.number);i.set(f.number,{number:f.number,url:w?.url||f.url,state:w?.state||"open",headRef:o,baseRef:f.baseRef,isDraft:w?.isDraft??!1,title:w?.title??""})}return{associatedPrs:[...i.values()],associatedBranches:f?[o]:[]}}catch(f){return console.warn(`[context] failed to gather existing work for issue #${h}; proceeding without it:`,f instanceof Error?f.message:f),{associatedPrs:[],associatedBranches:[]}}}async function Cn(n,h){let o=Number.parseInt(n.INFER_ISSUE_NUMBER??"",10);if(!Number.isFinite(o))throw Error("Missing or invalid INFER_ISSUE_NUMBER for PR context");let f=Number.parseInt(n.INFER_TRIGGERING_COMMENT_ID??"",10),a=Number.isFinite(f)?f:0,i=un(n),[s,u]=await Promise.all([h.getPullRequest(o),i?dn(n,h,o,a):h.listIssueComments(o).then((D)=>D.map((q)=>({id:q.id,author:q.author,body:q.body,createdAt:q.createdAt,isTrigger:a>0&&q.id===a})))]),w=`${h.owner}/${h.repoName}`,W=s.headRepoFullName!==""&&s.headRepoFullName!==w;return{kind:"pull_request",prNumber:o,prTitle:s.title,prBody:s.body,headRef:s.headRef,baseRef:s.baseRef,headRepoFullName:s.headRepoFullName,isFork:W,prState:s.state,triggeringCommentId:a,comments:u,...i?{reviewComment:i}:{}}}function un(n){let h=(n.INFER_REVIEW_COMMENT_PATH??"").trim();if(!h)return;let o=Number.parseInt(n.INFER_REVIEW_COMMENT_LINE??"",10),f=Number.parseInt(n.INFER_REVIEW_COMMENT_START_LINE??"",10);return{path:h,diffHunk:n.INFER_REVIEW_COMMENT_DIFF_HUNK??"",...Number.isFinite(o)?{line:o}:{},...Number.isFinite(f)?{startLine:f}:{}}}async function dn(n,h,o,f){let a=Number.parseInt(n.INFER_REVIEW_COMMENT_IN_REPLY_TO??"",10),i=[];if(Number.isFinite(a)&&a>0){let s=await h.listReviewComments(o);for(let u of s)if(u.id===a||u.inReplyToId===a)i.push({id:u.id,author:u.author,body:u.body,createdAt:u.createdAt,isTrigger:f>0&&u.id===f})}if(!i.some((s)=>s.isTrigger)){let s=M(n);if(s)i.push({id:s.id,author:s.author,body:s.body,createdAt:"",isTrigger:!0})}return i}function M(n){let h=n.INFER_TRIGGERING_COMMENT_ID??"",o=n.INFER_TRIGGERING_COMMENT_BODY??"",f=n.INFER_TRIGGERING_COMMENT_AUTHOR??"",a=Number.parseInt(h,10);if(!Number.isFinite(a)||a<=0)return;if(!o.trim())return;return{id:a,body:o,author:f}}var L=["GITHUB_TOKEN","OLLAMA_CLOUD_API_KEY","GROQ_API_KEY","LLAMACPP_API_KEY","OPENAI_API_KEY","CLOUDFLARE_API_KEY","COHERE_API_KEY","ANTHROPIC_API_KEY","DEEPSEEK_API_KEY","GOOGLE_API_KEY","MISTRAL_API_KEY","MINIMAX_API_KEY","MOONSHOT_API_KEY","NVIDIA_API_KEY","ZAI_API_KEY","OTEL_EXPORTER_OTLP_HEADERS","MEMORY_TOKEN","MEMORY_DEPLOY_KEY"],mn=["-----BEGIN [A-Z ]*PRIVATE KEY( BLOCK)?-----[\\s\\S]+?-----END [A-Z ]*PRIVATE KEY( BLOCK)?-----"],bn=["github_pat_[A-Za-z0-9_]{82,}","gh[pours]_[A-Za-z0-9]{36,}","AIza[0-9A-Za-z_-]{35}","xox[bpoa]-[A-Za-z0-9-]{20,}","sk-[A-Za-z0-9_-]{20,}","eyJ[A-Za-z0-9_-]+\\.eyJ[A-Za-z0-9_-]+\\.[A-Za-z0-9_-]{10,}"];var cn=/[.*+?^${}()|[\]\\]/g;function E(n,h,o=8){let f=[],a=new Set;for(let i of h){let s=n[i];if(typeof s!=="string")continue;if(s.trim().length<o)continue;if(a.has(s))continue;a.add(s),f.push(s)}return f}function wn(n){let h=new Set;for(let o of n){if(!o||h.has(o))continue;h.add(o),process.stdout.write(`::add-mask::${o}
`)}}function en(n={}){let h=n.placeholder??"***",o=n.minLength??8,f=n.env??process.env,a=n.heuristics??!1,i=E(f,L,o);i.sort((w,W)=>W.length-w.length);let s=i.map(xn);if(s.push(...mn),a)s.push(...bn);let u=s.length>0?new RegExp(s.join("|"),"g"):null;return{secretCount:i.length,redact(w){if(!u||!w)return w;return w.replace(u,h)}}}function xn(n){return n.replace(cn,"\\$&")}var V="/tmp/agent-output.txt",B="/tmp/infer-todos.json",t="/tmp/infer-cancelled";function Q(n){let h=process.env[n];if(!h)throw Error(`Missing required env var ${n}`);return h}function y(n){return process.env[n]??""}function yn(){let n=y("INFER_DRY_RUN")==="true",h=n?y("GITHUB_TOKEN"):Q("GITHUB_TOKEN"),o=Q("INFER_REPO"),f=y("INFER_ENABLE_GIT_OPERATIONS")!=="false",a=y("INFER_REDACT_HEURISTICS")==="true",i=E(process.env,L);wn(i);let s=en({env:process.env,heuristics:a}),u=y("INFER_COOKING_COMMENT_IS_REVIEW")==="true",w=new U({token:h,repo:o,redactor:s,dryRun:n,reviewComment:u});return{dryRun:n,token:h,repo:o,enableGitOps:f,enableHeuristics:a,redactor:s,github:w}}async function kn(n,h,o){try{return await an(n,h)}catch(f){if(o.failHard)throw f;return console.warn(`[${o.stepName}] context read failed (${f.message}); proceeding with env-derived data`),sn(n)}}var Wn={SYSTEM_DIRECT:`# Infer Agent (manual run)
...
Run \`git diff origin/{{baseRef}}...HEAD\` for the full diff and \`git log origin/{{baseRef}}..HEAD\` for the commit history.{{triggerSection}}`};function pn(n){let h=process.env[`INFER_PROMPT_OVERRIDE_${n}`];return h&&h.trim()?h:Wn[n]}function nh(n){let h=process.env[`INFER_PROMPT_OVERRIDE_${n}`];return h&&h.trim()?h:null}var hh={SYSTEM_ISSUE:["git commit","git push","gh pr create","gh pr ready","git status"],SYSTEM_DIRECT:["git commit","git push","gh pr create","gh pr ready","git status"],SYSTEM_PR:["git commit","git push","git status"],SYSTEM_PR_FORK:["git commit","git push"]};function oh(n){if(n.kind==="issue")return"SYSTEM_ISSUE";if(n.kind==="direct")return"SYSTEM_DIRECT";if(n.isFork)return"SYSTEM_PR_FORK";return"SYSTEM_PR"}function Rn(n){let h=oh(n),o=nh(h);if(o===null)return[];let f=hh[h];if(!f||f.length===0)return[];let a=f.filter((i)=>!o.includes(i));return a.length>0?[{key:h,missing:a}]:[]}function Y(n,h={}){return pn(n).replace(/\{\{(\w+)\}\}/g,(o,f)=>{if(!(f in h))throw Error(`Missing variable "${f}" for prompt "${n}"`);return String(h[f])})}function rn(n,h={}){if(n.kind==="issue")return uh(n);if(n.kind==="direct")return sh(n);return eh(n,h.diffStat??"")}var fh=`## A2A Agents

Delegated A2A tasks run in the background and you are notified automatically when they complete. Submit each task exactly once with A2A_SubmitTask, then WAIT for the completion notification - never resubmit the same task or poll for it in a loop. Only retry a submission if the submission call itself failed (e.g. the agent container was still starting).`,ah='## Images in the conversation\n\nIssue/PR text may embed images (`![...](url)` or `<img src="...">`) - screenshots, error dialogs, diagrams. These often carry the crux of the request, so read them BEFORE starting work: download each with WebFetch (`download=true`), then run ImageDecode on the saved file path, passing a `prompt` asking for what you need (e.g. the exact error text, the UI elements shown). If a download fails (private-repo attachments may be inaccessible), say so in your final response and continue with the text.';function Yn(n,h){let o=ih(n);if(process.env.INFER_A2A_ENABLED==="true")o=`${o}

${fh}`;if(process.env.INFER_VISION_ANNOTATOR_ENABLED==="true")o=`${o}

${ah}`;if(h.trim())return`${o}

## Additional Instructions

${h}`;return o}function ih(n){if(n.kind==="issue")return Y("SYSTEM_ISSUE",{issueNumber:n.issueNumber});if(n.kind==="direct")return Y("SYSTEM_DIRECT");if(n.isFork)return Y("SYSTEM_PR_FORK",{prNumber:n.prNumber,headRef:n.headRef,headRepoFullName:n.headRepoFullName,baseRef:n.baseRef});return Y("SYSTEM_PR",{prNumber:n.prNumber,headRef:n.headRef,baseRef:n.baseRef})}function sh(n){return Y("TASK_DIRECT",{prompt:n.prompt})}function uh(n){let h=n.triggeringComment?`

## Triggering comment from @${n.triggeringComment.author}

${n.triggeringComment.body}

Treat this comment as the user's most recent intent. If it asks for something more specific than the issue body, prioritise it.`:"";return Y("TASK_ISSUE",{issueNumber:n.issueNumber,issueTitle:n.issueTitle,issueBody:n.issueBody,existingWorkSection:wh(n),recentCommentsSection:qn((n.threadComments??[]).filter((o)=>!o.isTrigger),"Recent comments (chronological)"),triggeringCommentSection:h})}function qn(n,h){let o=n.filter((s)=>!s.author.endsWith("[bot]")),f=o.slice(-3);if(f.length===0)return"";let a=o.length-f.length,i=a>0?`_\u2026${a} earlier comment${a===1?"":"s"} omitted_
...
`)}function eh(n,h){let o=n.isFork?`
...
This is the user's most recent ask. Address it directly. Do not re-implement existing changes unless this comment asks for that.`:"",i=n.comments.filter((W)=>!W.isTrigger);if(n.reviewComment)return yh(n,n.reviewComment,{forkNotice:o,triggerSection:a,others:i});let s=qn(i,"Other comments (chronological)"),u=n.prBody.trim()?n.prBody:"_(no description)_",w=h.trim()?"```\n"+h.trim()+"\n```":"_(no changes on this branch yet)_";return Y("TASK_PR",{prNumber:n.prNumber,prTitle:n.prTitle,headRef:n.headRef,baseRef:n.baseRef,forkNotice:o,prBody:u,triggerSection:a,otherCommentsSection:s,diffStatSection:w})}function yh(n,h,o){let f=h.startLine&&h.line&&h.startLine!==h.line?`, lines ${h.startLine}-${h.line}`:h.line?`, line ${h.line}`:"",a=o.others.length>0?`
...
${n.body}`}function kh(n,h){let o=[],f=h.enableGitOps&&!(n.kind==="pull_request"&&n.isFork);if(o.push({name:"infer-action-context",hook:"pre_stream",trigger:"interval",interval:5,text:h.enableGitOps?Wh(n):"<system-reminder>Keep your TodoWrite plan current as you go. Only answering a question? Ignore this.</system-reminder>"}),f)o.push({name:"infer-action-wrap-up",hook:"pre_stream",trigger:"turns_before_max",threshold:10,text:Rh(n)}),o.push({name:"infer-action-failed-tool",hook:"post_tool",trigger:"on_failure",text:rh()});return o}function Wh(n){if(n.kind==="pull_request"&&n.isFork)return"<system-reminder>This PR is from a fork - you CANNOT commit or push. Investigate with file reads and git diff, then answer the user's question or summarise. Keep your TodoWrite plan current.</system-reminder>";if(n.kind==="pull_request")return`<system-reminder>Keep your TodoWrite plan current. If you changed files, commit + push after each step so PR #${n.prNumber} stays current - unpushed work is lost when the job ends. Only reviewing or answering? Do not change, commit, or push anything.</system-reminder>`;return"<system-reminder>Keep your TodoWrite plan current. Changing code? Work on a pushed branch with an open draft PR (`gh pr create --draft`) and commit + push after each step so nothing is lost - never commit on or push to main. Only answering a question? Ignore this.</system-reminder>"}function Rh(n){return`<system-reminder>You are close to the turn limit. Stop starting new work - if you have uncommitted or unpushed changes, commit and push them now ${n.kind==="pull_request"?`so PR #${n.prNumber} is up to date`:"and make sure the draft PR exists (`gh pr create --draft`)"}. Unpushed work is lost when the run ends. If you changed nothing, just finish your summary.</system-reminder>`}function rh(){return"<system-reminder>That tool call FAILED - the change did NOT happen. Re-read or re-check, fix it, and retry. Never mark a todo done or claim success on a failed call.</system-reminder>"}function Yh(n){let h=["enabled: true","merge: true","reminders:"];for(let o of n){if(h.push(`  - name: ${JSON.stringify(o.name)}`),h.push(`    hook: ${JSON.stringify(o.hook)}`),h.push(`    trigger: ${JSON.stringify(o.trigger)}`),o.interval!==void 0)h.push(`    interval: ${o.interval}`);if(o.threshold!==void 0)h.push(`    threshold: ${o.threshold}`);h.push(`    text: ${JSON.stringify(o.text)}`)}return h.join(`
`)+`
`}function Dn(n,h,o){let f=n.trim();if(f)return f.endsWith(`
`)?f:f+`
`;return Yh(kh(h,o))}import{execFileSync as qh}from"child_process";import{appendFileSync as $n,existsSync as uo,readFileSync as Oh,rmSync as Dh,writeFileSync as $h}from"fs";var Kh=60000;function Kn(){try{$h(t,"1")}catch(n){console.error("[runner] failed to write cancel marker:",n)}}function Hn(){try{Dh(t,{force:!0})}catch{}}function jn(n,h=l){try{return h(`git diff --stat origin/${Hh(n)}...HEAD`)}catch(o){return console.error("[runner] git diff --stat failed:",o),""}}function Jn(n,h=(o)=>o){try{let a=Oh(V,"utf8").split(`
`).filter((i)=>i.trim()!=="").slice(-n);if(a.length===0)return;console.error("=========================================="),console.error(`[recover] last ${a.length} line(s) of agent activity before it stopped:`),console.error("------------------------------------------");for(let i of a){let s=i.length>2000?i.slice(0,2000)+" \u2026":i;console.error(h(s))}console.error("==========================================")}catch(o){console.error("[recover] could not read agent transcript for breadcrumb:",o)}}function l(n){return qh("bash",["-c",n],{encoding:"utf8",timeout:Kh,env:{...process.env,GIT_TERMINAL_PROMPT:"0"}})}function Hh(n){return`'${n.replace(/'/g,"'\\''")}'`}function H(n,h){let o=process.env.GITHUB_OUTPUT;if(!o){console.log(`(would set output) ${n}=${h}`);return}if(h.includes(`
`)){let f=`_GHO_EOF_${Math.random().toString(36).slice(2)}`;$n(o,`${n}<<${f}
${h}
${f}
`)}else $n(o,`${n}=${h}
`)}function Zn(n){return typeof n==="object"&&n!==null&&n.role==="tool"&&typeof n.content==="string"}function Qn(n){if(typeof n!=="object"||n===null)return!1;let h=n.type;return h==="compaction_started"||h==="compaction_completed"}var Xn="Result of tool call: ";function Vn(n){if(!n.startsWith(Xn))return null;let h=n.slice(Xn.length);try{let o=JSON.parse(h);if(typeof o==="object"&&o!==null)return o;return null}catch{return null}}class A{handlers=new Map;flushers=[];listeners=[];on(n,h){return this.handlers.set(n,h),this}onMessage(n){return this.listeners.push(n),this}addFlusher(n){return this.flushers.push(n),this}async observe(n){for await(let h of n){for(let a of this.listeners)try{a(h)}catch(i){console.error("[ticker] message listener threw:",i)}if(!Zn(h))continue;let o=Vn(h.content);if(!o?.tool_name)continue;let f=this.handlers.get(o.tool_name);if(!f)continue;try{await f(o,h)}catch(a){console.error(`[ticker] handler for ${o.tool_name} threw:`,a)}}}async flush(){for(let n of this.flushers)try{await n()}catch(h){console.error("[ticker] flusher threw:",h)}}}function Pn(n,h){let o=null,f=null,a=null,i=async()=>{if(f=null,!o)return;let s=o.value;o=null,a=n(s).catch((u)=>{console.error("[throttle] fn threw:",u)}).finally(()=>{a=null}),await a};return{call(s){if(o={value:s},!f)f=setTimeout(()=>{i()},h)},async flush(){if(f)clearTimeout(f),f=null;if(o)await i();else if(a)await a}}}function zn(n){let h=Math.floor(n/1000);if(h<60)return`${h}s`;let o=Math.floor(h/60),f=h%60;if(o<60)return`${o}m ${f}s`;let a=Math.floor(o/60),i=o%60;return`${a}h ${i}m ${f}s`}var Qh=1500;async function Vh(){let{dryRun:n,enableGitOps:h,redactor:o,github:f}=yn(),a=y("INFER_REVIEW_MODE")==="true",i=h&&!a,s=y("INFER_COOKING_COMMENT_ID"),u=s?Number.parseInt(s,10):0,w=Number.isFinite(u)&&u>0,W=y("INFER_WORKFLOW_URL"),D=Q("INFER_AGENT_MODEL"),q=y("INFER_CUSTOM_INSTRUCTIONS"),Tn=y("INFER_BASH_ALLOW_APPEND"),_=y("INFER_LOGGING_DEBUG")==="true",P=on(process.env),R=await kn(process.env,f,{stepName:"dry-run",failHard:!n});if(R.kind==="pull_request"&&(h||a))try{Mh(R)}catch(e){if(!n)throw e;console.warn("[dry-run] PR head checkout failed; continuing on the current branch:",e instanceof Error?e.message:e)}let Fn=R.kind==="pull_request"?jn(R.baseRef):"",N=Yn(R,q),v=rn(R,{diffStat:Fn});if(i)for(let e of Rn(R)){let k=e.key.replace(/^SYSTEM_/,"").toLowerCase().replace(/_/g,"-");process.stdout.write(`::warning::INFER_PROMPT_OVERRIDE_${e.key} replaces the bundled system prompt (system-prompt-${k} / src/prompts/system-${k}.md) and is missing the git-safety markers: ${e.missing.join(", ")}. The default guards against lost work (branch-first, commit-per-todo, push, draft PR, finish checklist); your override dropped those instructions, so the agent may leave changes uncommitted or unpushed. Re-add them to your override, or use the custom-instructions input to layer extras on top of the default instead of replacing it.
`)}let Un=y("INFER_REMINDERS_CONFIG"),I=Dn(Un,R,{enableGitOps:i}),g=p(i,Tn),z=y("INFER_BIN")||"infer",Mn=y("INFER_NO_COLOR")==="true",S=Ph(process.env,{systemPrompt:N,bashAllowAppend:g,remindersYaml:I,reviewMode:a});console.log("=========================================="),console.log("SYSTEM PROMPT:"),console.log("==========================================");let C=y("INFER_BIN")?void 0:zh(z,S);if(console.log(C===void 0?N:Gh(C,_)),console.log("=========================================="),console.log(""),console.log("Running agent with task:"),console.log(v),console.log("---"),n)console.log("=========================================="),console.log("DRY RUN - the agent would be invoked with:"),console.log("=========================================="),console.log(`Model:        ${D}`),console.log(`Context kind: ${R.kind}`),console.log(`Git ops:      ${h?"enabled":"disabled"}`),console.log(`Review mode:  ${a?"enabled (Edit/Write/Delete disabled)":"disabled"}`),console.log(`INFER_BIN:    ${z}`),console.log("--- REMINDERS (INFER_REMINDERS_CONFIG) ---"),console.log(I),console.log("--- BASH ALLOW-LIST APPEND (added to the CLI read-only baseline) ---"),console.log(g||"(none - CLI read-only baseline only)"),console.log("==========================================");th(),Hn();let Ln=Date.now(),d=["agent","-m",D,v];if(Mn)d.push("--no-colors");let r=jh(z,d,{stdio:["inherit","pipe","pipe"],env:S});if(!r.stdout||!r.stderr)throw Error("child stdio not piped - this should not happen");let m=!1,b=!1,c=(e)=>{if(b)return;b=!0,m=!0,Kn(),console.error(`[runner] received ${e}; stopping the agent so the salvage step can recover its work`);try{r.kill("SIGKILL")}catch(k){console.error("[runner] failed to stop agent child:",k)}};process.once("SIGTERM",()=>c("SIGTERM")),process.once("SIGINT",()=>c("SIGINT"));let j=Xh(V),x=new Zh;if(r.stdout.pipe(j,{end:!1}),P.stdout)r.stdout.pipe(process.stdout,{end:!1});else console.log("[runner] agent stdout muted (set INFER_MIRROR_AGENT_LOGS=true to mirror); stderr still shown, full transcript written to /tmp/agent-output.txt");r.stdout.pipe(x),r.stdout.on("end",()=>j.end()),r.stderr.on("data",(e)=>{if(j.write(e),P.stderr)process.stderr.write(e)});let $=new A,J=w?Pn(async(e)=>{let k=Fh(e,W,D);try{await f.updateZone(u,"plan",k),console.log(`[ticker] updated plan section (${e.length} todos)`)}catch(En){console.error("[ticker] PATCH failed:",En)}},Qh):null;if(J)$.addFlusher(J.flush);else console.log("[ticker] no cooking comment; plan mirroring disabled (direct mode)");if($.on("TodoWrite",(e)=>{let k=e.data?.todos;if(!Array.isArray(k))return;if(Bh(k),J)J.call(k)}),_)$.onMessage((e)=>{if(Qn(e)){console.log(e.type==="compaction_started"?"[agent] context compaction started (summarising older turns)\u2026":"[agent] context compaction completed");return}let k=e;if(k.role==="user"&&k.hidden===!0&&k.kind==="system_reminder")console.log("[agent] system reminder injected")});await $.observe(fn(x)),await $.flush();let K=await Lh(r),G=Date.now()-Ln;if(console.log(""),console.log("=========================================="),console.log(`Agent exited with code ${K}`),console.log(`Duration: ${zn(G)}`),console.log("=========================================="),m){if(H("run-duration-ms",String(G)),await Eh(j),P.stdout)Jn(40,o.redact);return console.error("[runner] cancelled mid-run; the salvage step will recover any work and report the timeout"),130}return H("exit-code",String(K)),H("run-duration-ms",String(G)),H("result",K===0?"Agent completed successfully":`Agent failed with exit code ${K}`),K}function Ph(n,h){return{...n,INFER_PROMPTS_AGENT_SYSTEM_PROMPT:h.systemPrompt,INFER_AGENT_SYSTEM_PROMPT_WITH_DEFAULTS:"true",INFER_TOOLS_BASH_ALLOW_APPEND:h.bashAllowAppend,INFER_REMINDERS_CONFIG:h.remindersYaml,...h.reviewMode?{INFER_TOOLS_EDIT_ENABLED:"false",INFER_TOOLS_WRITE_ENABLED:"false",INFER_TOOLS_DELETE_ENABLED:"false"}:{},OTEL_EXPORTER_OTLP_ENDPOINT:n.OTEL_EXPORTER_OTLP_ENDPOINT??"",OTEL_EXPORTER_OTLP_HEADERS:n.OTEL_EXPORTER_OTLP_HEADERS??"",OTEL_SERVICE_NAME:n.OTEL_SERVICE_NAME??"infer-action",OTEL_RESOURCE_ATTRIBUTES:n.OTEL_RESOURCE_ATTRIBUTES??""}}function zh(n,h){try{let o=Jh(n,["debug","agent","system_prompt"],{env:h,encoding:"utf8",timeout:30000});if(o.status!==0||!o.stdout?.trim())return;return o.stdout}catch{return}}function Gh(n,h){if(h)return n;let o=n.indexOf("PERSISTENT MEMORY INDEX");if(o===-1)return n;let f=n.slice(o),a=f.indexOf(`
...
${o.join(" \xB7 ")}`}function Fh(n,h,o){let f=Th(h,o);if(n.length===0)return`${f}
...
`)}function Uh(n){let h=n.stderr;return((typeof h==="string"?h:"")+String(n.message)).includes("couldn't find remote ref")}function Mh(n,h=l){try{if(n.isFork){let o=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${o}`),h(`git fetch origin pull/${n.prNumber}/head:${o}`),h(`git checkout ${o}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{h(`git fetch origin ${n.headRef}`)}catch(o){if(!Uh(o))throw o;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),h(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}h(`git checkout ${n.headRef}`)}}catch(o){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:o})}}async function Lh(n){if(n.exitCode!==null)return n.exitCode;return new Promise((h)=>{n.on("close",(o)=>h(o??0))})}async function Eh(n){if(n.writableFinished)return;await Promise.race([new Promise((h)=>n.once("finish",h)),new Promise((h)=>setTimeout(h,2000).unref())])}function Bh(n){try{Gn(B,JSON.stringify(n))}catch(h){console.error("[runner] failed to persist todos:",h)}}function th(){try{Gn(B,"[]")}catch{}}if(import.meta.main)Vh().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{zh as resolveMergedSystemPrompt,Fh as renderPlan,Gh as redactMemoryIndex,Mh as ensurePrHeadCheckedOut,Ph as buildChildEnv};
