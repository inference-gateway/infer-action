...
`)}function Wo(n){let o=n.stderr;return((typeof o==="string"?o:"")+String(n.message)).includes("couldn't find remote ref")}function Yo(n,o=V){try{if(n.isFork){let h=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${h}`),o(`git fetch origin pull/${n.prNumber}/head:${h}`),o(`git checkout ${h}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{o(`git fetch origin ${n.headRef}`)}catch(h){if(!Wo(h))throw h;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),o(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}o(`git checkout ${n.headRef}`)}}catch(h){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:h})}}async function qo(n){if(n.exitCode!==null)return n.exitCode;return new Promise((o)=>{n.on("close",(h)=>o(h??0))})}async function Ho(n){if(n.writableFinished)return;await Promise.race([new Promise((o)=>n.once("finish",o)),new Promise((o)=>setTimeout(o,2000).unref())])}function $o(n){try{$n(P,JSON.stringify(n))}catch(o){console.error("[runner] failed to persist todos:",o)}}function Do(){try{$n(P,"[]")}catch{}}if(import.meta.main)yo().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{Ro as renderPlan,Yo as ensurePrHeadCheckedOut,lo as buildChildEnv};
