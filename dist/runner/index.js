...
`)}`:"";return q("TASK_PR_REVIEW",{prNumber:n.prNumber,prTitle:n.prTitle,headRef:n.headRef,baseRef:n.baseRef,forkNotice:o.forkNotice,filePath:h.path,lineInfo:f,diffHunk:h.diffHunk,threadSection:a,triggerSection:o.triggerSection})}function $n(n){return`**@${n.author}** \xB7 ${n.createdAt}
...
`)?f:f+`
`;return $h(Yh(h,o))}import{execFileSync as rh}from"child_process";import{appendFileSync as Dn,existsSync as eo,readFileSync as Dh,rmSync as Kh,writeFileSync as Hh}from"fs";var jh=60000;function Kn(){try{Hh(l,"1")}catch(n){console.error("[runner] failed to write cancel marker:",n)}}function Hn(){try{Kh(l,{force:!0})}catch{}}var jn=2;function Jn(n,h=t){try{return h(`git diff --stat origin/${Jh(n)}...HEAD`)}catch(o){return console.error("[runner] git diff --stat failed:",o),""}}function Zn(n,h=(o)=>o){try{let a=Dh(V,"utf8").split(`
`).filter((i)=>i.trim()!=="").slice(-n);if(a.length===0)return;console.error("=========================================="),console.error(`[recover] last ${a.length} line(s) of agent activity before it stopped:`),console.error("------------------------------------------");for(let i of a){let s=i.length>2000?i.slice(0,2000)+" \u2026":i;console.error(h(s))}console.error("==========================================")}catch(o){console.error("[recover] could not read agent transcript for breadcrumb:",o)}}function t(n){return rh("bash",["-c",n],{encoding:"utf8",timeout:jh,env:{...process.env,GIT_TERMINAL_PROMPT:"0"}})}function Jh(n){return`'${n.replace(/'/g,"'\\''")}'`}function D(n,h){let o=process.env.GITHUB_OUTPUT;if(!o){console.log(`(would set output) ${n}=${h}`);return}if(h.includes(`
`)){let f=`_GHO_EOF_${Math.random().toString(36).slice(2)}`;Dn(o,`${n}<<${f}
${h}
${f}
`)}else Dn(o,`${n}=${h}
`)}function Xn(n){return typeof n==="object"&&n!==null&&n.role==="tool"&&typeof n.content==="string"}function Vn(n){if(typeof n!=="object"||n===null)return!1;let h=n.type;return h==="compaction_started"||h==="compaction_completed"}var Qn="Result of tool call: ";function Pn(n){if(!n.startsWith(Qn))return null;let h=n.slice(Qn.length);try{let o=JSON.parse(h);if(typeof o==="object"&&o!==null)return o;return null}catch{return null}}class A{handlers=new Map;flushers=[];listeners=[];on(n,h){return this.handlers.set(n,h),this}onMessage(n){return this.listeners.push(n),this}addFlusher(n){return this.flushers.push(n),this}async observe(n){for await(let h of n){for(let a of this.listeners)try{a(h)}catch(i){console.error("[ticker] message listener threw:",i)}if(!Xn(h))continue;let o=Pn(h.content);if(!o?.tool_name)continue;let f=this.handlers.get(o.tool_name);if(!f)continue;try{await f(o,h)}catch(a){console.error(`[ticker] handler for ${o.tool_name} threw:`,a)}}}async flush(){for(let n of this.flushers)try{await n()}catch(h){console.error("[ticker] flusher threw:",h)}}}function zn(n,h){let o=null,f=null,a=null,i=async()=>{if(f=null,!o)return;let s=o.value;o=null,a=n(s).catch((u)=>{console.error("[throttle] fn threw:",u)}).finally(()=>{a=null}),await a};return{call(s){if(o={value:s},!f)f=setTimeout(()=>{i()},h)},async flush(){if(f)clearTimeout(f),f=null;if(o)await i();else if(a)await a}}}function Gn(n){let h=Math.floor(n/1000);if(h<60)return`${h}s`;let o=Math.floor(h/60),f=h%60;if(o<60)return`${o}m ${f}s`;let a=Math.floor(o/60),i=o%60;return`${a}h ${i}m ${f}s`}var Ph=1500;async function zh(){let{dryRun:n,enableGitOps:h,redactor:o,github:f}=yn(),a=y("INFER_REVIEW_MODE")==="true",i=h&&!a,s=y("INFER_COOKING_COMMENT_ID"),u=s?Number.parseInt(s,10):0,w=Number.isFinite(u)&&u>0,W=y("INFER_WORKFLOW_URL"),K=X("INFER_AGENT_MODEL"),O=y("INFER_CUSTOM_INSTRUCTIONS"),Un=y("INFER_BASH_ALLOW_APPEND"),v=y("INFER_LOGGING_DEBUG")==="true",P=on(process.env),Y=await kn(process.env,f,{stepName:"dry-run",failHard:!n});if(Y.kind==="pull_request"&&(h||a))try{Bh(Y)}catch(e){if(!n)throw e;console.warn("[dry-run] PR head checkout failed; continuing on the current branch:",e instanceof Error?e.message:e)}let Ln=Y.kind==="pull_request"?Jn(Y.baseRef):"",N=qn(Y,O),_=Rn(Y,{diffStat:Ln});if(i)for(let e of Yn(Y)){let k=e.key.replace(/^SYSTEM_/,"").toLowerCase().replace(/_/g,"-");process.stdout.write(`::warning::INFER_PROMPT_OVERRIDE_${e.key} replaces the bundled system prompt (system-prompt-${k} / src/prompts/system-${k}.md) and is missing the git-safety markers: ${e.missing.join(", ")}. The default guards against lost work (branch-first, commit-per-todo, push, draft PR, finish checklist); your override dropped those instructions, so the agent may leave changes uncommitted or unpushed. Re-add them to your override, or use the custom-instructions input to layer extras on top of the default instead of replacing it.
`)}let Mn=y("INFER_REMINDERS_CONFIG"),I=rn(Mn,Y,{enableGitOps:i,maxTurns:Number.parseInt(y("INFER_AGENT_MAX_TURNS"),10)||0}),S=p(i,Un),z=y("INFER_BIN")||"infer",Tn=y("INFER_NO_COLOR")==="true",g=Gh(process.env,{systemPrompt:N,bashAllowAppend:S,remindersYaml:I,reviewMode:a});console.log("=========================================="),console.log("SYSTEM PROMPT:"),console.log("==========================================");let C=y("INFER_BIN")?void 0:Fh(z,g);if(console.log(C===void 0?N:Uh(C,v)),console.log("=========================================="),console.log(""),console.log("Running agent with task:"),console.log(_),console.log("---"),n)console.log("=========================================="),console.log("DRY RUN - the agent would be invoked with:"),console.log("=========================================="),console.log(`Model:        ${K}`),console.log(`Context kind: ${Y.kind}`),console.log(`Git ops:      ${h?"enabled":"disabled"}`),console.log(`Review mode:  ${a?"enabled (Edit/Write/Delete disabled)":"disabled"}`),console.log(`INFER_BIN:    ${z}`),console.log("--- REMINDERS (INFER_REMINDERS_CONFIG) ---"),console.log(I),console.log("--- BASH ALLOW-LIST APPEND (added to the CLI read-only baseline) ---"),console.log(S||"(none - CLI read-only baseline only)"),console.log("==========================================");Ah(),Hn();let Bn=Date.now(),d=["agent","-m",K,_];if(Tn)d.push("--no-colors");let R=Zh(z,d,{stdio:["inherit","pipe","pipe"],env:g});if(!R.stdout||!R.stderr)throw Error("child stdio not piped - this should not happen");let m=!1,b=!1,c=(e)=>{if(b)return;b=!0,m=!0,Kn(),console.error(`[runner] received ${e}; stopping the agent so the salvage step can recover its work`);try{R.kill("SIGKILL")}catch(k){console.error("[runner] failed to stop agent child:",k)}};process.once("SIGTERM",()=>c("SIGTERM")),process.once("SIGINT",()=>c("SIGINT"));let j=Xh(V),x=new Vh;if(R.stdout.pipe(j,{end:!1}),P.stdout)R.stdout.pipe(process.stdout,{end:!1});else console.log("[runner] agent stdout muted (set INFER_MIRROR_AGENT_LOGS=true to mirror); stderr still shown, full transcript written to /tmp/agent-output.txt");R.stdout.pipe(x),R.stdout.on("end",()=>j.end()),R.stderr.on("data",(e)=>{if(j.write(e),P.stderr)process.stderr.write(e)});let H=new A,J=w?zn(async(e)=>{let k=Mh(e,W,K);try{await f.updateZone(u,"plan",k),console.log(`[ticker] updated plan section (${e.length} todos)`)}catch(En){console.error("[ticker] PATCH failed:",En)}},Ph):null;if(J)H.addFlusher(J.flush);else console.log("[ticker] no cooking comment; plan mirroring disabled (direct mode)");if(H.on("TodoWrite",(e)=>{let k=e.data?.todos;if(!Array.isArray(k))return;if(th(k),J)J.call(k)}),v)H.onMessage((e)=>{if(Vn(e)){console.log(e.type==="compaction_started"?"[agent] context compaction started (summarising older turns)\u2026":"[agent] context compaction completed");return}let k=e;if(k.role==="user"&&k.hidden===!0&&k.kind==="system_reminder")console.log("[agent] system reminder injected")});await H.observe(fn(x)),await H.flush();let $=await Eh(R),G=Date.now()-Bn;if(console.log(""),console.log("=========================================="),console.log(`Agent exited with code ${$}`),console.log(`Duration: ${Gn(G)}`),console.log("=========================================="),m){if(D("run-duration-ms",String(G)),await lh(j),P.stdout)Zn(40,o.redact);return console.error("[runner] cancelled mid-run; the salvage step will recover any work and report the timeout"),130}if(D("exit-code",String($)),D("run-duration-ms",String(G)),$===jn)return D("result","Agent stopped early (reached the turn limit)"),console.error("[runner] agent reached max turns; the salvage step will recover any unpushed work and the report renders a stopped-early status"),0;return D("result",$===0?"Agent completed successfully":`Agent failed with exit code ${$}`),$}function Gh(n,h){return{...n,INFER_PROMPTS_AGENT_SYSTEM_PROMPT:h.systemPrompt,INFER_AGENT_SYSTEM_PROMPT_WITH_DEFAULTS:"true",INFER_TOOLS_BASH_ALLOW_APPEND:h.bashAllowAppend,INFER_REMINDERS_CONFIG:h.remindersYaml,...h.reviewMode?{INFER_TOOLS_EDIT_ENABLED:"false",INFER_TOOLS_WRITE_ENABLED:"false",INFER_TOOLS_DELETE_ENABLED:"false"}:{},OTEL_EXPORTER_OTLP_ENDPOINT:n.OTEL_EXPORTER_OTLP_ENDPOINT??"",OTEL_EXPORTER_OTLP_HEADERS:n.OTEL_EXPORTER_OTLP_HEADERS??"",OTEL_SERVICE_NAME:n.OTEL_SERVICE_NAME??"infer-action",OTEL_RESOURCE_ATTRIBUTES:n.OTEL_RESOURCE_ATTRIBUTES??""}}function Fh(n,h){try{let o=Qh(n,["debug","agent","system_prompt"],{env:h,encoding:"utf8",timeout:30000});if(o.status!==0||!o.stdout?.trim())return;return o.stdout}catch{return}}function Uh(n,h){if(h)return n;let o=n.indexOf("PERSISTENT MEMORY INDEX");if(o===-1)return n;let f=n.slice(o),a=f.indexOf(`
...
${o.join(" \xB7 ")}`}function Mh(n,h,o){let f=Lh(h,o);if(n.length===0)return`${f}
...
`)}function Th(n){let h=n.stderr;return((typeof h==="string"?h:"")+String(n.message)).includes("couldn't find remote ref")}function Bh(n,h=t){try{if(n.isFork){let o=`pr-${n.prNumber}`;console.log(`[runner] fork PR; fetching pull/${n.prNumber}/head into ${o}`),h(`git fetch origin pull/${n.prNumber}/head:${o}`),h(`git checkout ${o}`)}else{console.log(`[runner] checking out PR head branch ${n.headRef}`);try{h(`git fetch origin ${n.headRef}`)}catch(o){if(!Th(o))throw o;process.stdout.write(`::warning::PR head branch ${n.headRef} no longer exists on origin (likely deleted when the PR was closed or merged); recreating it from pull/${n.prNumber}/head. If the PR is closed or merged, pushing will not reopen it.
`),h(`git fetch origin pull/${n.prNumber}/head:${n.headRef}`)}h(`git checkout ${n.headRef}`)}}catch(o){throw Error(`Failed to check out PR head (${n.headRef}). Aborting before spawning the agent so it doesn't run against the wrong branch.`,{cause:o})}}async function Eh(n){if(n.exitCode!==null)return n.exitCode;return new Promise((h)=>{n.on("close",(o)=>h(o??0))})}async function lh(n){if(n.writableFinished)return;await Promise.race([new Promise((h)=>n.once("finish",h)),new Promise((h)=>setTimeout(h,2000).unref())])}function th(n){try{Fn(E,JSON.stringify(n))}catch(h){console.error("[runner] failed to persist todos:",h)}}function Ah(){try{Fn(E,"[]")}catch{}}if(import.meta.main)zh().then((n)=>process.exit(n),(n)=>{console.error("[runner] uncaught error:",n),process.exit(1)});export{Fh as resolveMergedSystemPrompt,Mh as renderPlan,Uh as redactMemoryIndex,Bh as ensurePrHeadCheckedOut,Gh as buildChildEnv};
