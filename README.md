...
5. **Agent Execution**: The agent runs with your specified model (or the
   ...
   take over entirely (a project-committed `.infer/reminders.yaml` still takes
...
| `reminders-config`              | Verbatim reminders YAML written straight to `~/.infer/reminders.yaml`, REPLACING the composed default. Use to take full control of the CLI's native reminders (hooks, triggers, cadences). A non-empty value bypasses `reminder-interval` / `wrap-up-threshold`; you must re-cover any built-in behavior you want kept (loaded file replaces the CLI's built-in list). See the CLI `config/reminders.go` for the schema | No       | `''`           |
| `reminder-interval`             | Turns between the periodic context reminder (the pre_stream nudge re-stating branch-first / commit-per-todo / push / draft-PR workflow). Only used when `reminders-config` is empty                                                                                                                                                                                                                                     | No       | `5`            |
| `wrap-up-threshold`             | Turns-before-max threshold for the wrap-up reminder that tells the agent to stop starting new work and commit/push now. Only used when `reminders-config` is empty                                                                                                                                                                                                                                                      | No       | `10`           |
