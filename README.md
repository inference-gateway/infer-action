...
5. **Agent Execution**: The agent runs with your specified model (or the
   ...
   native reminders (passed via `INFER_REMINDERS_CONFIG`, CLI >= v0.129.0): a
   periodic nudge to keep the branch, draft PR, and pushes current, a wrap-up
   reminder near the turn limit telling the agent to commit and push everything
   now, and a `post_tool` `on_failure` nudge that re-states the "a failed call
   means the change did not happen" rule after each _failed_ tool call on
   writable runs. When `memory-repo` is set, the CLI's built-in
   memory-consult/memory-hygiene reminders are re-emitted (a supplied reminders
   config replaces the built-in list, so the action duplicates them). Power
   users can pass a full `reminders-config` YAML to take over entirely
...
| `reminders-config`              | Verbatim reminders YAML passed to the CLI via `INFER_REMINDERS_CONFIG`, REPLACING the composed default. Use to take full control of the CLI's native reminders (hooks, triggers, cadences). You must re-cover any built-in behavior you want kept (a supplied config replaces the CLI's built-in list). Needs CLI >= v0.129.0; see the CLI `config/reminders.go` for the schema | No       | `''`           |
