...
| ---------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `true` (default) | empty                         | Temporary local collector; traces fan back to the CLI so the result footer shows the full distributed tree |
...
| `version`                     | Infer CLI version to install                                                                                                                                                                                                                                                                                                                                                                                         | No       | `v0.187.0`                 |
...
| `enable-run-judge`            | Run an LLM-as-a-judge pass over the finished run: one judge call fed by the transcript digest (tool failures analysed first, per-tool counts, loop signals - no raw logs), scoring the trajectory `ok`/`degraded`/`blocked` with blockers and improvement suggestions in the result footer, job summary, and the `judge-score`/`judge-verdict` outputs. Best-effort - a judge failure never affects the run's status | No       | `false`                    |
| `judge-model`                 | Model for the run judge in `provider/model` format. Defaults to the model the run used                                                                                                                                                                                                                                                                                                                               | No       | `''`                       |
...
| `judge-score`             | Run judge score when `enable-run-judge` is true: `ok`, `degraded`, or `blocked` (empty if the judge failed or the flag is off)                            |
| `judge-verdict`           | Run judge verdict as JSON (`{score, blockers, improvements}`) when `enable-run-judge` is true (empty if the judge failed or the flag is off)              |
