...
| `mirror-agent-logs`     | Mirror agent stdout/stderr to the workflow log; defaults to false (suppressed). Set true to mirror the transcript. The `/tmp/agent-output.txt` file that post-results reads for the comment footer is always written. A minimal heartbeat still prints. | No       | `false`    |
