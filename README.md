...
Use the `apt` input to install system packages before the agent runs
(Debian/Ubuntu runners only):
...
- uses: inference-gateway/infer-action@main
  with:
    ...
    model: anthropic/claude-opus-5
    anthropic-api-key: ${{ secrets.ANTHROPIC_API_KEY }}
    apt: libwebkit2gtk-4.1-dev libxdo-dev libssl-dev
```

The downloaded `.deb` files are cached with `actions/cache`, keyed on the
runner OS, architecture, and the normalized package list. Repeat runs with
the same list restore the packages from the cache instead of re-downloading
them from the apt mirrors; changing the list produces a new cache key and
installs the new set. Caching is best-effort: any cache restore or save
failure falls back to a normal uncached install.
...
**Private fork example.** Redirect the install shorthand, on-demand downloads, and the
catalog index to a private fork in one setting:
...
When `skills-repository` is set, bare skill names like `maintainer` resolve against
...
- **A `name:tag` pair** - pins a specific OCI image tag of a first-party agent
  (e.g. `browser-agent:lightpanda`); maps to `infer agents add <name> --tag <tag>`.
...
| ---------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `true` (default) | empty                         | Temporary local collector; traces fan back to the CLI so the result footer shows the full distributed tree (when `debug: true`) |
...
### Local collector (`otel-collector: true`, the default)
...
inside the agents. The `Traces` (and `Stats`) footer sections render only when
`debug: true` - the example below sets it:
...
uploaded with the run artifact and embedded inline in the result comment (when
`png`/`jpg`/`jpeg`/`gif`/`webp` and within the 10-image / 5 MB caps; other
formats are listed as files).
...
- The result comment gets an **Artifacts** section: `png`, `jpg`, `jpeg`, `gif`
  and `webp` images are embedded inline; everything else - including `svg`,
  which is in the default `artifact-extensions` but is not rendered inline by
  GitHub's camo proxy - is listed with sizes next to a download link for the
  run artifact.
- To make images render in the comment, the action pushes them to an
  ...
  serve inline images. The push happens on private repositories too; only a
  failed push (fork PRs, read-only tokens) makes images fall back to the
  plain file list. Up to 10 images of at most 5 MB each are embedded per run.
...
Ready-to-run example workflows and the
`task test:issue | test:comment | test:review | test:direct` helpers live in [`examples/local/`](examples/local) - see
...
7. **Result Posting**: The action posts a final summary to the same issue
   comment with:
   - Status icon and exit code - success / failure / stopped early
     (timeout, max-turns limit, salvaged or otherwise unfinished work)
   ...
   - Any failed tool calls (collapsed; shown when `debug: true`)
...
| `github-app-slug`             | Slug of the GitHub App whose bot identity authors the agent's commits (e.g. `infer-bot`); resolved via `GET /users/{slug}[bot]` (skipped when `github-app-user-id` is provided). Falls back to `github-actions[bot]` when empty or on failure                                                                                                                                                                             | No       | `''`                       |
| `github-app-user-id`          | Numeric user id of the GitHub App bot (`<slug>[bot]`); used directly to build the commit author email, skipping the `GET /users/{slug}[bot]` lookup. Prefer passing it from a step that already resolved it (the reusable workflow does) so attribution does not depend on a second API call                                                                                                                              | No       | `''`                       |
...
| `version`                     | Infer CLI version to install                                                                                                                                                                                                                                                                                                                                                                                              | No       | `v0.208.0`                 |
| `ollama-cloud-api-key`        | Ollama Cloud API key                                                                                                                                                                                                                                                                                                                                                                                                      | No\*     | -                          |
| `groq-api-key`                | Groq API key                                                                                                                                                                                                                                                                                                                                                                                                              | No\*     | -                          |
| `llamacpp-api-key`            | llama.cpp API key                                                                                                                                                                                                                                                                                                                                                                                                         | No\*     | -                          |
...
| `llamacpp-api-url`            | llama.cpp API URL                                                                                                                                                                                                                                                                                                                                                                                                         | No\*     | -                          |
| `ollama-api-url`              | Ollama API URL                                                                                                                                                                                                                                                                                                                                                                                                            | No\*     | -                          |
| `max-turns`                   | Maximum agent iterations                                                                                                                                                                                                                                                                                                                                                                                                  | No       | `150`                      |
| `custom-instructions`         | Additional instructions appended to default behavior                                                                                                                                                                                                                                                                                                                                                                      | No       | `''`                       |
| `system-prompt-issue`         | Override the bundled system prompt for issue triggers. Substitutes `{{issueNumber}}`. REPLACES (not merges with) the default; an override missing the git-safety block emits a `::warning::`. See [Overriding System Prompts](#overriding-system-prompts)                                                                                                                                                                 | No       | `''`                       |
| `system-prompt-pr`            | Override the bundled system prompt for PR-comment triggers on same-repo PRs. Substitutes `{{prNumber}}`, `{{headRef}}`                                                                                                                                                                                                                                                                                                    | No       | `''`                       |
| `system-prompt-pr-fork`       | Override the bundled system prompt for PR-comment triggers on fork PRs (view-only). Substitutes `{{prNumber}}`, `{{headRef}}`, `{{headRepoFullName}}`, `{{baseRef}}`                                                                                                                                                                                                                                                      | No       | `''`                       |
| `system-prompt-direct`        | Override the bundled system prompt for direct-prompt (`workflow_dispatch`) runs. No variables                                                                                                                                                                                                                                                                                                                             | No       | `''`                       |
| `task-issue`                  | Override the bundled task template for issue triggers. Substitutes `{{issueNumber}}`, `{{issueTitle}}`, `{{issueBody}}`, `{{triggeringCommentSection}}`, `{{existingWorkSection}}`, `{{recentCommentsSection}}`                                                                                                                                                                                                           | No       | `''`                       |
| `task-pr`                     | Override the bundled task template for PR-comment triggers. Substitutes `{{prNumber}}`, `{{prTitle}}`, `{{headRef}}`, `{{baseRef}}`, `{{forkNotice}}`, `{{prBody}}`, `{{triggerSection}}`, `{{otherCommentsSection}}`, `{{diffStatSection}}`, `{{ciChecksSection}}`                                                                                                                                                       | No       | `''`                       |
| `task-direct`                 | Override the bundled task template for direct-prompt runs. Substitutes `{{prompt}}`                                                                                                                                                                                                                                                                                                                                       | No       | `''`                       |
...
| `agents`                      | Comma/newline-separated list of A2A agents to run as local Docker containers (first-party names like `browser-agent`, `name:tag` pairs like `browser-agent:lightpanda`, or `name=oci-image` pairs). Registers + enables each, turns on A2A, and defaults them to the main `model`. Requires Docker. See [Spinning up A2A Agents](#spinning-up-a2a-agents)                                                                 | No       | `''`                       |
...
| `apt`                         | Newline- or space-separated list of apt packages to install before the agent runs (Debian/Ubuntu runners only). Downloaded `.deb` files are cached with `actions/cache`, keyed on the runner OS, architecture, and the normalized package list; repeat runs with the same list skip the re-download, and cache failures fall back to a normal uncached install. See [Apt Packages](#apt-packages)                         | No       | `''`                       |
...
| `no-color`                    | Disable colored output from Infer CLI commands (passes `--no-colors` and sets `NO_COLOR=1`); avoids raw ANSI escape sequences (mojibake) in the result comment from piped command output                                                                                                                                                                                                                                  | No       | `false`                    |
